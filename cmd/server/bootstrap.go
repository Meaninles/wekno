...
// bootstrapEnvVar is the env var that names the username of the user who
...
//     username is auto-promoted, with no extra ops step.
...
const bootstrapEnvVar = "WEKNORA_BOOTSTRAP_SYSTEM_ADMIN_USERNAME"
...
func runStartupBootstrap(c *dig.Container) {
	...
	username := strings.TrimSpace(os.Getenv(bootstrapEnvVar))
	if username == "" {
		return
	}
	...
	if err := c.Invoke(func(userSvc interfaces.UserService) {
		bootstrapSystemAdmin(ctx, userSvc, username)
	}); err != nil {
		logger.Warnf(ctx, "[bootstrap] failed to resolve UserService: %v", err)
	}
}

// bootstrapSystemAdmin promotes the user identified by `username` to system
...
// every error path.
//
// The bootstrap intentionally does NOT create a user when the username is
...
func bootstrapSystemAdmin(ctx context.Context, userSvc interfaces.UserService, username string) {
	user, err := userSvc.GetUserByUsername(ctx, username)
	if err != nil {
	...
	if total > 0 {
		logger.Infof(ctx,
			...
			bootstrapEnvVar, username, total, user.ID)
		return
	}
	...
	if err := userSvc.UpdateUser(ctx, user); err != nil {
		logger.Warnf(ctx,
			...
			bootstrapEnvVar, username, user.ID, err)
		return
	}
	logger.Infof(ctx,
		"[bootstrap] promoted user %s (%s) to system admin via %s",
		user.ID, username, bootstrapEnvVar)
}
