...
## 九、提示词冻结后的追加回归与当前发布状态

在最后一次通用提示词修改之后，新增了独立的 Helix 实验室样本交接用例，而不是
复用采购、培训或既有用例后替换名词。该用例混合知识库证据、用户事实、未知状态、
旧值废弃、精确原话追问、英文限域输出、知识问答中的否定表达和不同对象的外部操作
边界。`run-20260901-144019-reviewed.json` 的结果为：

- 3 个独立 session、30/30 个轮次完成；
- production 与 assisted 30/30 逐字相同，repair trigger 为 0；
- SSE 最终公开答案与持久化/历史答案 30/30 逐字相同；
- production 和 assisted 的完整会话 Codex 审核均为 3/3 PASS；
- 第 2 次运行漏列一条仍有效的“不要发送或保存”边界，但没有执行或声称执行该动作，
  因此按最低标准记为 `minor_issue` 而不是追求模板化的完美答案；
- 唯一未过项是 quick-answer P95 30,316ms，高于既有 30,000ms 上限 316ms；门禁阈值
  没有被放宽或删除。

此后没有继续围绕该漏项增加提示词。原因是现有领域无关契约已经要求保存
actor/action/object/destination/modality/scope；为一次未造成越权的摘要遗漏继续加码，
会增加模板化和时延风险，违背“允许小瑕疵、避免过拟合”的目标。

当前运行时代码候选为 `694dddeb5df48ff5aa8328d15d3b93a4f4559933`，共享镜像为
`sha256:068665769a1ec72532c3d8864248303ec5f682370395c673682a574811d15918`。
`preflight-20260901-150459-dev.json` 对 4 个全新领域 case、12 个独立 session、三个
智能体和每个 13 轮的历史窗口溢出检查均为 READY；数据集 52 个轮次中 required
claims、Judge rubric 和 reference answers 的数量均为 0。

正式执行产物为 `run-20260901-150714.json`，审核后产物和门禁为：

- `run-20260901-151144-reviewed.json`
- `gate-20260901-151144.json`
- `report-20260901-151144.md`

本轮 12/12 个 session 都在首轮因同一个外部模型提供方的 TLS/传输故障终止，剩余
144 个轮次按协议跳过；没有完成的 production answer，Eval 修复触发、模型调用、
工具调用和新增耗时均为 0。Codex 对 production 与 assisted 共 24 份完整会话审核均
标为 `INVALID/not_applicable`：无可审查答案，不能把基础设施故障判成语义 PASS 或
FAIL。production release gate 因 invalid rate、执行覆盖和重复通过率给出
`INVALID/NOT_READY`，assisted 没有挽救 production。

同一容器访问其他 HTTPS 服务可以正常完成 TLS 握手；当前模型端点在系统默认、
关闭证书校验的诊断环境、TLS 1.2 和 TLS 1.3 下均在握手阶段 EOF，因此没有采用关闭
校验、改用 HTTP、换模型或降低超时/门禁阈值的规避方式。所有已配置的可用模型也
指向同一端点，不能通过切换现有模型得到独立验证。

追加验证结果：Eval 框架 173/173、通用智能体 114/114、相关 Go 包全部通过；新增的
记录器失败注入测试连续运行 20 次通过，证明异步导出被阻塞并最终失败时，生产答案
和 HTTP 状态仍立即原样返回。隔离运行编排 15/15 个角色健康，API、前端、移动端和
两个 sidecar 均可访问；main 编排保持停止，main 工作区未被本任务覆盖。

**截至本节更新，代码整改与新增回归已完成，但发布批准仍为 NOT_READY。** 必须在
外部模型端点恢复后，使用同一冻结数据、模型和门禁重新完成三智能体 12 个 session，
由 Codex 逐个审核可用的完整对话；随后还需按正式 v3 策略完成冻结 baseline、gate
和不可见 sealed holdout。不能用本次 INVALID 结果或 Helix 单智能体通过替代这些
发布证据。
