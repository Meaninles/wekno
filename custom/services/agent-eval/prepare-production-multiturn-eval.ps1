...
switch ($Stage) {
    "experiment" {
        $common.Split = "dev"
        $common.Manifest = "/workspace/manifests/production-multiturn-ready.v1-experiment.manifest.json"
        $common.Policy = "/workspace/policies/production-multiturn-experiment-gate.v1.json"
    }
    "optimization" {
        $common.Split = "dev"
        $common.Manifest = "/workspace/manifests/production-multiturn-ready.v1-optimization.manifest.json"
        $common.Policy = "/workspace/policies/production-multiturn-optimization-gate.v1.json"
    }
    "release" {
        $common.Split = "gate"
        $common.Manifest = "/workspace/manifests/production-multiturn-ready.v1-release.manifest.json"
    ...
    "sealed" {
        ...
        $common.Manifest = "/workspace/manifests/production-multiturn-holdout.v1.manifest.json"
