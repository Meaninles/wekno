...
JUDGE_SYSTEM_PROMPT = (
    ...
    "actions. Accept semantically explicit forms such as 'will not send on the user's behalf', 'no execution "
    "authorization', and 'not yet determined' without demanding one contiguous answer-key phrase. When "
    "require_scoped_sections is true, facts must also appear in the correct section. Rules in "
    "forbidden_claims, forbidden_inferences, forbidden_unknown_facts, and forbidden_recommendations fail when "
    "the answer asserts the prohibited meaning. A denial of a category does not assert examples named only to "
    "clarify that denial: for example, 'there is no change-level classification such as major/general' must not "
    "fail a rule that forbids inventing major/general levels. An unnegated assertion elsewhere still fails. "
    "Explicitly audit every conversation-state rule; never pass "
