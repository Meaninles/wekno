...
class AnswerProtocol(MiddlewareBase):
    async def on_model_call(self, agent, input_kwargs, next_handler):
        # Keep the native SDK schema and execution. Describe its actual role
        # here: final submission is also valid without collecting resources.
...
class Answer(BaseModel):
    ...
    answer: str = Field(description="Complete user-facing answer in the requested language and format, including supporting citations and relevant limitations. For document claims, use current evidence handles; recover original evidence with read_conversation before rewriting earlier cited facts.")
