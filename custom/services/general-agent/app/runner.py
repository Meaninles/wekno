...
from .schemas import ChatPayload, ChatResult, RunEvent, RuntimeToolSpec, SidecarArtifact
...
async def run_eval_focused_evidence_retrieval(
    ...
    if canonical == "list_knowledge_chunks":
        ...
        # user-derived; a later ordinary model turn may refine if this misses.
        arguments = {"query": "；".join(searches[:5])[:800]}

    state[RETRIEVAL_TOOL_CALLS_STATE_KEY] = calls + 1
    state["eval_focused_retrieval_attempted"] = True
    try:
        result = await asyncio.to_thread(call_tool_callback, payload, spec.name, arguments)
    except Exception as exc:
        state["eval_focused_retrieval_error"] = type(exc).__name__
        return False
...
def _turn_contract_relevance_features(query: str, issues: list[dict[str, Any]]) -> dict[str, int]:
    ...
    for target in [
        ...
        *required_evidence_searches(query),
...
class GeneralAgentRunner:
    ...
    async def run(self) -> AsyncIterator[RunEvent]:
        ...
        if prompt_observation:
            ...
            prompt_observation["eval_focused_retrieval_attempted"] = bool(
                turn_contract_state.get("eval_focused_retrieval_attempted")
            )
            prompt_observation["eval_focused_retrieval_recovered"] = bool(
                turn_contract_state.get("eval_focused_retrieval_recovered")
            )
