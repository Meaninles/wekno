...
- Drafts, plans, templates, and sample text may organize supported facts but must omit missing operational details or mark them neutral/unknown. Do not invent a duration, lifecycle state, actor, destination, channel, or completed step for completeness.
...
def build_prompt(
    ...
    parts.append(
        ...
        "Counts and outcomes, including zero, absent records, and analysis requests do not establish unasserted lifecycle state. Keep operation boundaries active for the same task until the user explicitly changes them, and do not treat a content edit as authorization for a tool or artifact. "
        "Drafts and summaries must omit unsupported operational details or mark them neutral/unknown. Claim a search, retrieval, read, save, send, update, or other operation only when a matching current-turn result establishes it. "
