...
function Invoke-FinalDeliveryApi {
    param(
        [string]$Method,
        [string]$Path,
        [object]$Body = $null,
...
function Get-FinalDeliveryAuthChallenge {
    $response = Invoke-FinalDeliveryApi -Method Get -Path "/api/v1/custom/auth-security/challenge"
    Assert-FinalDelivery ($response.StatusCode -eq 200) "local authentication challenge is available"
    ...
    Assert-FinalDelivery ($answer.Length -gt 0) "local SVG challenge contains an answer"
    return @{
...
function Protect-FinalDeliveryPassword {
...
function New-FinalDeliveryLogin {
    ...
    $password = "LocalFinalDelivery!" + [Guid]::NewGuid().ToString("N").Substring(0, 18)
    ...
    $challenge = Get-FinalDeliveryAuthChallenge
    $encrypted = Protect-FinalDeliveryPassword -PlainText $password -PublicKey $challenge.PublicKey
    $password = $null
    $response = Invoke-FinalDeliveryApi -Method Post -Path "/api/v1/auth/login" -Body @{
        username = $AdminUsername
        encrypted_password = $encrypted
        challenge_id = $challenge.Id
        captcha_answer = $challenge.Answer
    }
    Assert-FinalDelivery ($response.StatusCode -eq 200) "encrypted local administrator login succeeds"
    $payload = $response.Content | ConvertFrom-Json
    Assert-FinalDelivery (
...
function Restore-FinalDeliveryPassword {
...
try {
    $token = New-FinalDeliveryLogin
    $env:WEKNORA_FINAL_DELIVERY_E2E_TOKEN = $token
    $env:WEKNORA_FINAL_DELIVERY_E2E_BASE_URL = $BaseUrl
    $env:WEKNORA_FINAL_DELIVERY_E2E_CONCURRENCY = [string]$Concurrency
    $token = $null

    & node (Join-Path $PSScriptRoot "api_final_delivery.mjs")
    if ($LASTEXITCODE -ne 0) {
        throw "final-delivery API acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
    Remove-Item Env:WEKNORA_FINAL_DELIVERY_E2E_TOKEN -ErrorAction SilentlyContinue
    Remove-Item Env:WEKNORA_FINAL_DELIVERY_E2E_BASE_URL -ErrorAction SilentlyContinue
    Remove-Item Env:WEKNORA_FINAL_DELIVERY_E2E_CONCURRENCY -ErrorAction SilentlyContinue
    Restore-FinalDeliveryPassword
}
