...
> 生产平台入口为 `https://knora.moutai.com.cn`。Lite 仅适合本地单机使用；生产平台的
> 多租户、共享空间和完整服务能力按标准版运行，不能用 Lite 的本地端口或单进程命令替代。
...
若你不需要多团队协作、复杂解析流水线与多服务架构，Lite 更适合个人或小团队在本机零依赖试用；需要共享空间、多租户与完整解析引擎矩阵时，请使用标准版。
