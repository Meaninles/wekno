...
> 代码核对日期：2026-09-13。
>
> 生产访问统一使用 [https://knora.moutai.com.cn](https://knora.moutai.com.cn)：
> API 基地址为 `https://knora.moutai.com.cn/api/v1`，移动端为
> `https://knora.moutai.com.cn/mobile/`。本目录中出现的本地地址必须明确标注为
> “仅本地开发/验收”，不能拿来作为生产配置。
...
| 前端/业务用户 | [用户使用指南](./custom/使用指南/用户使用指南.md) | [系统使用说明](./custom/系统使用说明.md)、[知识图谱](./KnowledgeGraph.md) |
| 智能体开发 | [智能体开发指南](./custom/使用指南/智能体开发指南.md) | [智能体能力说明](./custom/WeKnora智能体能力使用文档.md)、[通用运行时](./custom/通用智能体方案.md) |
...
- [生产版用户与开发者使用指南包说明](./custom/使用指南/使用指南包说明.md)

综合说明直接使用 Markdown 维护。修改架构、部署、状态或主要用户交互时，需要同步
更新并执行链接、配置值和实现一致性检查；面向外部使用者的示例优先写生产基地址，
只有开发部署章节才写本地地址。
...
生产基地址、API 路径、角色数量和内置智能体以当前代码、路由、配置模板和 Helm
值文件交叉核对；不把提交记录当作唯一依据。部署节点、内部 Service、对象存储、
模型网关和任何凭据均不在公开文档中展开，现场值只能放在受保护的部署配置中。
