...
> 历史本地问题修复与样本验收记录，仅用于追溯；当前实现、生产部署和地址以源码/配置及
> [当前实现架构与文档索引](./当前实现架构与文档索引.md)为准。生产入口统一使用
> `https://knora.moutai.com.cn`；本文中的 `localhost` 仅表示本地验收。业务文件名、内容、
> 账号、数据源和证据路径不应作为生产信息使用。
...
一个生产样本只有约 170 KB、约 92 行、1,344 个非空单元格，真实内容最远到 `BC`（第 55 列），但工作表 XML 中第 6 行记录到 `XFB`（第 16,382 列）。第 100 列以后有 16,284 个只带样式、值为空的单元格；文件名不写入版本化文档。
...
对照样本虽然有 16.24 MB、约 47 万行，但最多只有 11 列，是长窄表，只需 25 个物理 part，因此能够正常处理；业务文件名不写入版本化文档。
...
- 小型宽表样本：旧算法 10,770 parts，新算法 5 parts。
...
2026-08-02 使用原失败文档样本执行真实 API 重建：
