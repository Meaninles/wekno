...
> 当前实现交付版，代码与配置核对日期：2026-09-13。
> 本文说明每类智能体的特点、资源组合、工作区运行时、产物对象存储和双副本验收。
> 配置细节见[智能体开发指南](./使用指南/智能体开发指南.md)。

> 生产入口：桌面 Web 为 `https://knora.moutai.com.cn/`，API 为
> `https://knora.moutai.com.cn/api/v1`，移动端为
> `https://knora.moutai.com.cn/mobile/`。文档不记录内部节点、Service、对象存储、
> 模型网关或任何凭据；本地地址只在明确标注为本地开发时出现。
...
| 类型 | 适用场景 | 关键能力 |
|---|---|---|
| 知识问答（`knowledge-qa`） | 制度、手册、产品资料 | RAG、引用、稳定回答；默认 15 次迭代 |
| Wiki 问答（`wiki-qa`） | 体系化知识阅读 | 页面、目录、链接关系 |
| Wiki + RAG 混合（`hybrid-rag-wiki`） | 页面与原文联合回答 | Wiki 导航、分块检索、精确引用 |
| 数据分析（`data-analysis`） | 经营/业务数据库 | 只读 SQL、指标、图表 |
| 表格分析（`table-analysis`） | CSV/Excel | schema、DuckDB、图表 |
| 通用智能体（`general-agent`） | 综合流程 | 知识、DB、MCP、技能、Web、产物 |
| 文档处理（`document-processing-agent`） | 办公文档 | Word、Excel、PDF、PPT |
| 知识库管理（`knowledge-base-manager`） | 受限文档维护 | 选定知识库内的新增、替换、删除 |
| 自定义（`custom`） | 特殊业务流程 | 手动配置提示词、工具和资源 |

## 2. 两类运行链路

### 2.1 统一 Agent Harness

所有智能体配置统一使用 `agent` 运行模式。知识问答、Wiki 和混合类型按能力选择
检索与工具；其他类型可使用相同 Harness 的工作区、工具和产物能力。类型差异由
`agent_type` 与配置的工具、资源、技能、MCP、联网和产物开关决定。

### 2.2 专业 Agent 工作区运行时

数据分析、表格分析、通用和文档处理可以使用独立的 Agent 工作区运行时。当前实现由
Go 业务服务负责鉴权、上下文和工具回调，由 AgentScope Python Harness 负责模型编排与
产物交付：
...
`wiki-qa` 适合页面化知识、目录和概念关系；`hybrid-rag-wiki` 在 Wiki 导航后继续
检索分块，以提供精确原文引用。两者都需要绑定具备相应 Wiki 能力的知识库。
...
启用产物的 Agent Runtime 类型使用持久化产物交付流程；实际数量、大小和上传边界
由当前后端配置及租户策略共同限制，不能把某个本地测试值当成生产 SLA。
...
两个 Agent Runtime 副本部署在受保护的生产工作节点上并按主机名打散。滚动更新
`maxSurge=0,maxUnavailable=1`；具体节点名和内部地址只存在于受保护的现场 values，
不写入使用文档。
