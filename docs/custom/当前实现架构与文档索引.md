...
- 每个 app 副本都能完成一份文档的全部解析与衍生阶段，不存在只做 Wiki 或只做
  向量化的专用 app。
...
flowchart LR
    ...
    Delivery --> Claim["任一空闲 app 领取完整文档"]
...
- 3 个 app、每实例完整文档并发为 4 时，最多同时接纳 12 份完整文档。
...
每个 app 具有稳定 `instance_id`，每次进程启动生成新的 `boot_id`：
...
模型准入是 Redis 共享的“集群总量”，不是每个 app 各自拥有一套：
...
管理员设置中的 `asynq.concurrency` 是“每个 app 的完整文档工作流并发”。生产值
为 4，三个 app 的集群完整文档容量为 12。内部子任务线程和模型集群准入是不同
层次，不能通过把三者设置为同一个大数字来扩容。
...
| 节点 | 规格 | 工作负载 |
|---|---|---|
| `10.14.201.1` | 8C/32Gi，500G 数据盘 | app、DocReader、general-agent、document-processing-agent、frontend、mobile-web 各 1 |
| `10.14.201.2` | 8C/32Gi，500G 数据盘 | 与 `.1` 相同 |
...
|---|---:|---:|---:|---:|---:|
| app | 3 | 完整文档 4 | 1.5C / 3Gi | 4C / 8Gi | 80Gi |
...
app 和 DocReader 使用 `.1/.2/.7` 主机名拓扑打散，Agent/Web 使用 `.1/.2` 打散。
大临时卷工作负载滚动更新必须 `maxSurge=0, maxUnavailable=1`，避免临时出现第四个
100Gi 解析卷导致本地卷池不足。
...
- app、DocReader、两个 Agent、桌面前端和移动前端均有多副本。
- 文档工作流可在安全证明旧执行终止后由其他 app 接管。
...
因此当前实现可以水平扩展文档处理并抵御执行层单 Pod/单节点故障，但不能宣称
...
| `internal/custom/modules/modeladmission/` | 按资源/模型/租户的 Redis 集群级准入 |
| `internal/custom/modules/workretry/` | VLM/Wiki 长耗时模型任务的有限次数和真实调用计数 |
| `internal/custom/modules/documentsplit/` | 超大文档拆分、租约、重试和代表性采样 |
...
- 其他旧测试知识库保留 22 条历史 open processing span；其当前 generation 已终态，
  generation/attempt fencing 会排除旧记录。若要清理，应另开维护任务，不要在本次
  文档或生产迁移中直接改用户/历史测试数据。
