...
`grep_chunks` 的 PostgreSQL 同步有效切片检索投影、迁移与一致性边界见
[GrepChunks 同步检索投影](./GrepChunks同步检索投影.md)。仅本地实现，不代表生产已部署。

Agent Runtime Kubernetes 存储已改为单静态 PVC + 每运行独立 subPath；跨轮输入、
执行回执和交付协议保持。配置、回收及验证边界见[静态存储说明](./Agent运行时静态PVC与目录隔离.md)。

> 适用代码基线：当前工作区，核对日期 2026-08-12。
> 本文描述已经落地并经过测试的实现，不是未来路线图。生产更新必须同时阅读
> [当前生产实现与部署基线](./当前生产实现与部署基线.md)和
...
1. [当前生产实现与部署基线](./当前生产实现与部署基线.md)：现网版本来源、实际
   拓扑、资源、模型并发、存储和发布约束。
2. 本文：当前代码的总体架构、组件边界、关键语义和文档入口。
3. [当前版本生产更新部署执行手册](./当前版本生产更新部署执行手册.md)：
   针对现有生产 CCE 的逐步迁移、部署、验收和回滚命令。
4. [生产集群无 RWX 最优部署方案](./生产集群无RWX最优部署方案.md)：
   目标拓扑、资源与容量依据、存储设计和单点边界。
5. [文档解析水平扩展与故障恢复](./文档解析水平扩展与故障恢复.md)：
   文档级持久队列、状态机、租约、fencing、接管和专项测试。
6. [`helm/values-production-ha.yaml`](../../helm/values-production-ha.yaml)：
   与上述生产方案对应的机器可渲染配置基线。
7. [容量与调度统一控制](./容量与调度统一控制.md)：
   实际模型资源池、有效值编译、跨模块匹配、等待/失败语义和 API 验收。
8. [聊天模型会话队列与生产配置](./聊天模型会话队列与生产配置.md)：
   会话级模型队列、配置层级、用户状态、本地并发结果和生产配置。
9. 功能/API/测试文档：描述具体入口、字段、操作和验收。

智能体进度、来源展开和事件回放见[统一思考过程展示](./统一思考过程展示.md)，
桌面、移动端与嵌入聊天复用同一展示机制，不增加模型调用。
10. [通用智能体多副本回调与生产验收](./通用智能体多副本回调与生产验收.md)：
    API 进程内 run 的回调亲和、生产配置、三副本验收和精确回滚。
...
- 知识问答、Wiki、数据分析、表格分析、通用智能体和文档处理智能体。
- [统一运行预算与文件交付](./统一运行预算与文件部分交付.md)：基于本轮基线的纯代码产物校验、发布与恢复。
...
flowchart TB
    ...
    subgraph Workers["可水平扩展执行层"]
        ...
        GeneralAgent["agent-runtime 多副本"]
        Models["外部 llmgateway / Chat / Embedding / Rerank / VLM / Omni"]
    end
...
- DocReader 为每次解析创建独立进程组，硬超时或取消时终止整个进程树，异常文档
  不得永久占住 gRPC worker。
- 所有 IM 适配器的最终问答正文通过同一个确定性引用出口；模型保存统一来源标签，
  出口只按真实消息引用生成文档分片、Wiki 和网页三类可点击目标；文档与 Wiki 使用
  同一签名设备自适应入口，电脑端和移动端分别进入各自独立的匿名精确来源页；文档
  原文访问仍绑定同一能力，普通知识库和聊天访问不放宽认证。模型正文的制度条款
  表达与平台可点击引用是两套共存体系，不能互相覆盖。
...
模型准入是 Redis 共享的“集群总量”，不是每个 API/worker Pod 各自拥有一套。当前
生产有效值如下；完整模型目录、后台份额和已验证上限见
[当前生产实现与部署基线](./当前生产实现与部署基线.md)：

| 资源池 | 集群执行并发 | 交互预留 | 最大等待 | 单租户 | 单文档 |
|---|---:|---:|---:|---:|---:|
...
聊天另有会话级 FIFO。当前两个主要聊天池共执行 48、等待 52，总接纳上限 100；
100 已包含执行中的 48。已接受 waiter 通过 SSE 自动晋升；个人满和池满分别返回
不同结构化 429，并在消息创建前拒绝。完整边界见
...
| app/DocReader 解析工作区 | Pod/节点本地临时盘 | 节点隔离 hostPath scratch | 可丢弃 |
| Agent SDK/Office 工作区 | Pod/节点本地临时盘 | 节点隔离 hostPath scratch | 可丢弃 |
| 状态、chunk、向量、问题、Wiki | PostgreSQL | PostgreSQL/ParadeDB | 持久 |
| 实体和关系 | Neo4j | Neo4j | 持久 |
| 投递、流、准入租约 | Redis | 外部 Redis | 可恢复的运行状态 |

生产不使用 RWX。当前 scratch 位于 `.1/.2/.7` 的
`/mnt/weknora-data/weknora-v2-scratch/<role>`，按角色和 Pod 隔离。对象存储也不能
挂载成解析或 Office 的 POSIX 工作目录；大量小文件、随机写、rename 和目录遍历
必须在本地临时盘完成。
...
所有问答使用同一 AgentScope Harness。各智能体保留独立类型和能力：知识问答固定 15 次，其余类型固定 50 次。Go 持久化 Run、检查点、工具回执与事件；Worker 使用租约和 epoch 接管，最终消息与结果原子提交。工作区独立于 Worker，最终产物经 Go 校验并写入私有对象存储。详见[统一 Agent Harness 实现与验证](./统一AgentHarness实现方案.md)。
...
| `10.14.201.6` | 8C/16Gi | 现有 PostgreSQL 和集群基础设施；不承载应用 worker |
...
|---|---:|---:|---:|---:|---:|
| API | 3 | SSE/API | 500m / 1280Mi | 1500m / 3Gi | 仅临时附件 |
| parse-worker | 3 | 完整文档 4 | 1C / 2Gi | 3C / 5Gi | hostPath scratch |
| derivative-worker | 2 | consumer 18 | 500m / 1Gi | 1500m / 2Gi | hostPath scratch |
| wiki-worker | 2 | consumer 6 | 500m / 1Gi | 1500m / 2Gi | hostPath scratch |
| maintenance | 2 | PG advisory 主备 | 150m / 384Mi | 750m / 1Gi | 无 |
| DocReader | 3 | gRPC worker 4，进程隔离 | 750m / 1Gi | 4C / 4Gi | hostPath scratch |
| agent-runtime | 2 | 单 Run 单所有者，可租约接管 | 250m / 512Mi | 2C / 2Gi | 独立工作区 |
...
跨不同工作节点，Agent/Web 使用 `.1/.2` 打散。资源预算和数据库连接预算以
...
| 外部 llmgateway | 集群外部依赖 | 网关不可用时模型调用停止；集群内 LiteLLM 当前不承载流量 |
...
| `internal/custom/modules/iam/` | SSO、组织/人员同步、本地用户映射、同步计划和异常人员跳过记录 |
...
### 12.1 本地 runtime profile

本地开发后端使用 `custom/tests/runtime_profile_e2e/docker-compose.yml` 的分角色
编排，API 入口为 `http://localhost:8080`。启动前由 `docker-compose.dev.yml`
提供 PostgreSQL、Redis、MinIO、Neo4j 和 1 个基础 DocReader；运行时角色为 API 3、
解析 2、衍生任务 2、Wiki 2、维护 2、一次性迁移 1、运行时 DocReader 2，另有 API
入口和 DocReader 入口各 1 个。后端代码修改后的停止、共享镜像构建和重新创建命令
见[快速开发模式](../wiki/开发部署/快速开发模式.md)。
...
- 历史验收环境的 LiteLLM 请求 `/v1/responses/input_tokens` 时收到 405，随后使用本地
  tokenizer 兼容回退；该记录只解释旧测试。当前生产流量走外部 llmgateway 域名，
  集群内 LiteLLM 不承载流量。
...
- [统一 Agent Harness 实现与验证](./统一AgentHarness实现方案.md)
