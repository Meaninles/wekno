...
所有应用副本都注册全部处理器，不需要按 Wiki、图谱或向量化拆成不同类型的专用 Pod。一个文档的衍生 delivery 可能由任一同构副本消费，但它们都受同一个 workflow、processing generation 和写入 fencing 约束；文档 owner 持有的 admission 槽位会等到该完整工作流达到终态才释放。
...
- Kubernetes verifier 对精确 Pod UID 的当前 app container `terminated` 状态或 terminal Pod phase 的确认。
...
3. 原文和中间文件位于共享对象存储，或所有 app/DocReader 可见的 RWX 文件系统。
...
使用 `STORAGE_TYPE=local` 时，默认 RWO PVC 不能安全支撑多 app 副本。生产环境优先使用对象存储；必须使用文件系统时，应明确选择支持 ReadWriteMany 的存储类。
...
| local RWO 存储配多 app | 文件不可被所有实例一致访问 | 不支持；改用对象存储或 RWX |
