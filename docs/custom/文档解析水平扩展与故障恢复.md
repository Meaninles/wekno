...
> 历史验收记录中的测量结果仅用于追溯；当前实现核对日期：2026-09-13。生产角色、资源和并发以
> [当前生产实现与部署基线](./当前生产实现与部署基线.md)为准。
> 生产入口统一使用 `https://knora.moutai.com.cn`；本文中的 `localhost` 仅表示本地验收。
...
当前生产部署三个 parse-worker 和三个 DocReader，并按生产调度约束分布到多个节点：
...
`<PROTECTED_SCRATCH_PATH>/<role>` 下隔离 hostPath，只承载可重算数据。
...
以下结果来自历史本地验收证据。原始报告不提交版本库，具体文件名、指纹和本地路径不写入版本化文档，不能替代 CI 中可追溯的制品归档。

| 能力 | 报告 | 结果摘要 |
|---|---|---|
| 最终综合验收 | 历史本地报告 | 多实例解析、全部启用衍生、持久队列、故障恢复、状态筛选、预览、图谱和真实召回均完成过验收；具体副本与资源以当前生产基线为准 |
| 历史模型容量 | 历史本地报告 | 当时的成功率、P95 和建议容量仅用于追溯；当前生产容量以受保护配置和生产基线为准 |
| 状态机、PostgreSQL/Redis、竞态 | 历史本地报告 | 高水位活性边界、并发回收、重启/终止/恢复交错、Redis 契约和 Python Harness 完成过验证 |
| 向量、摘要、问题、图谱、Wiki | 历史本地报告 | 各阶段状态和事件一致性完成过验证；原始事件文件只保存在本地验收产物中 |
| 严格同 workload 单实例与三实例 | 历史本地报告 | 相同合成工作负载下，1→3 个 healthy 实例的吞吐约由 1.23 提升到 4.50 docs/s；不能外推为生产 SLA |
| 多实例大批量文档 | 历史本地报告 | 合成文档批次全部完成，单实例并发和三实例总吞吐按当时配置核对 |
| 图片 OCR/caption | 历史本地报告 | OCR、caption、summary 和正文均完成过存在性核对，不记录样本正文 |
| CSV/XLSX 表格派生 | 历史本地报告 | 表格摘要、列信息、派生文本和状态完成过核对，不记录样本值 |
| PDF、浏览器排队和恢复 | 历史本地报告 | 普通 PDF、队列位置展示、同实例重启、跨实例接管、暂停边界、Redis/API/全实例重启均完成过测试 |

早期单实例与三实例报告只能引用各自原始吞吐，不能把不同文档规模直接计算成扩展倍数。相同合成工作负载、大小、上传并发、知识库配置和验证范围下，实例数由 1 增至 3 时实测吞吐约为 1.23→4.50 docs/s；单次本地结果会受缓存、调度和依赖抖动影响，不能把该效率外推成生产容量承诺。

后续综合验收已经补齐 Word、Excel、PPT、PDF、图片、音频和其他测试集格式，
并在开启向量、摘要、问题生成、知识图谱和 Wiki 的条件下检查完成性。保留的真实
示例验收知识库 `<ID>` 的文档完整性如下（业务名称、数量和标识不写入版本化文档）：

- 所有样本的 `parse_status`、`summary_status`、`enrichment_status` 和 `wiki_status` 均为完成，
  `pending_subtasks=0`。
- PostgreSQL、向量索引、Wiki 和 Neo4j 的数量由本地验收脚本交叉核对，具体业务数据不写入版本化文档。
- 所有持久工作流均完成，没有非终态记录。

另有数据资料和全格式样本均完成。该结果是当前
测试环境的可复核基线，不代表所有未来未知/损坏文件都必然成功，也不能外推为
生产 SLA。已通过验收的业务知识库应保留继续使用，不得为重复专项测试删除或无理由重建。
...
- [ ] 每份文档的成功由 PostgreSQL、向量、Neo4j、Wiki、对象存储和前端联合核对，
      不是只相信 runner 的汇总状态。
- [ ] 已通过验收的业务知识库保持原样，其他测试失败只重建对应测试知识库。
