...
> 当前实现核对日期：2026-08-06。生产角色、资源和并发以
...
代码支持完整实例承载全部处理器，但当前生产已经拆为专用运行角色：
...
生产以同一 app 镜像分别运行 `api`、`parse-worker`、`derivative-worker`、
`wiki-worker` 和 `maintenance`。这是运行职责隔离，不是不同代码分支；workflow、
processing generation、模型资源池和写入 fencing 仍然全集群共享。开发环境可以使用
`dev-all`，生产禁止用它替代专用角色。
...
当前生产部署三个 parse-worker 和三个 DocReader，并在 `.1/.2/.7` 跨主机调度：
...
- 为 parse-worker 注入 namespace 和 Pod UID 组成的稳定 instance ID。
...
- 提供 parse-worker 与 DocReader 的 topology spread 和可选 PDB。
- 为 parse-worker 配置足够的 termination grace period。

扩容前必须满足：

1. PostgreSQL 已完成工作流 migration，所有角色使用同一个数据库。
2. 所有角色使用同一个 Redis delivery 集群。
3. 原文和已提交的解析持久对象位于共享对象存储；app、DocReader 和 Agent 的中间工作目录使用各 Pod 独立、可丢弃的本地临时盘。
4. 所有角色连接相同且具备容量的向量库、Neo4j 和模型服务。
5. `asynq.concurrency` 已按单 Pod CPU、内存、DocReader 和下游配额确定。
6. API Service、Ingress 或负载均衡器只把用户流量发往多个 ready API 副本。

扩容 parse-worker 后，ready 实例会自动参与全局 queued workflow 的 claim。缩容应
使用正常 SIGTERM 和足够的 grace period；不要先强删 Pod 再依赖 heartbeat 超时接管。

本项目生产固定使用私有 OBS，不依赖 RWX；知识对象、Agent 产物和 Agent 原文件交付
使用三个互不相同的部署级命名空间前缀。当前 Pod 临时工作区使用
`/mnt/weknora-data/weknora-v2-scratch/<role>` 下隔离 hostPath，只承载可重算数据。

更多 chart 参数见 [Helm README](../../helm/README.md)。

## DocReader 毒文档与存量连接保护

DocReader 的 readiness 只能把异常 Pod 从新连接 endpoint 中摘除，不能自动切断调用端
已经建立的 gRPC/TCP 长连接。如果旧连接仍指向异常 endpoint，健康 DocReader 可能
空闲，而全部 parse 槽仍在等待旧连接超时。

当前实现增加第二道、有界保护：每次 DocReader 解析在独立 OS 进程组运行，超过
`DOCREADER_REQUEST_TIMEOUT_SECONDS=600` 或请求取消后终止整个进程组，等待
`DOCREADER_PROCESS_KILL_GRACE_SECONDS=3` 后强制清理子进程。健康探针使用独立 gRPC
端口和线程池，不与业务 worker 竞争。故障处理还应滚动或重建调用端连接，让新 RPC
只落到健康 endpoint。

验收不能只看队列数字短暂下降，应同时证明：

- 异常 PDF/Office 文件只占用一个有界窗口，超时后 worker 可以接收新请求；
- 小 PDF、DOCX、XLSX、图片等任务持续进入健康 DocReader；
- 三个 DocReader 都收到真实请求且没有长期无进展的 parser 子进程；
- parse workflow 租约持续续期/完成，不按统一 30 分钟波次反复超时；
- 旧 generation 或旧 boot 的迟到结果被 fencing，不能覆盖新结果。

历史遗留租约如果属于已被替换的旧 generation，应先备份证据，再以明确的
`superseded` 终态收敛，不能删除、盲目重放或消耗业务失败预算。
...
| 工作流 | state、stage、lease age、last progress、dispatch epoch、lease epoch、execution attempts、failure attempts | 长时间无进展、真实失败预算耗尽、epoch 异常回退 |
...
| 同稳定实例容器重启 | 新 boot 采用旧租约并递增 lease/dispatch epoch；执行诊断次数可增加，业务失败次数不变 | 是，但依赖运行时唯一 instance ID |
| PostgreSQL/ParadeDB 关键词索引损坏 | 全局断路、parse-worker 暂停接单，maintenance 校验并并发重建索引，复验后自动恢复 | 单索引仍是共享故障域，但不会再逐文档耗尽失败预算 |
...
| 一个 DocReader 停止 | readiness 摘除新 endpoint；单请求进程隔离有界清理毒文档；调用端旧 gRPC 长连接必须失效后才完全转向健康副本 | 三副本可恢复，但仍需验证存量连接 |
...
| 共享本地工作目录 | 多 Pod 互相覆盖临时文件 | 不支持；生产持久文件用 OBS，hostPath scratch 按角色和 Pod 隔离 |
...
| 能力 | 报告 | 结果摘要 |
|---|---|---|
| 最终综合验收 | [20260726-0107](../../custom/tests/document_processing_cluster_e2e/final_acceptance_outputs/20260726-0107/final_acceptance_report.json) | 多实例解析、全部启用衍生、持久队列、故障恢复、状态筛选、预览、图谱和真实召回全部通过；验收时 5 个 app、3 个 DocReader，无重启/OOM |
| DeepSeek V4 Flash 历史容量 | [20260726-deepseek-v4-flash](../../custom/tests/model_capacity_reports/20260726-deepseek-v4-flash.json) | 当时 32/32 成功、P95 5.655 秒并曾建议取 24；当前生产 V4 Flash INT8 上限为 16，以容量基线为准 |
| 状态机、真实 PostgreSQL/Redis、race | `20260723-012042`（历史本地制品，未入库） | 固定高水位活性边界、16 coordinator 并发回收、重启/终止/恢复三方交错、Redis 契约和 Python harness 均通过 |
| 向量、摘要、问题、图谱、Wiki | `20260722-232224`（历史本地制品，未入库） | 3/3 完成；详细产物曾保存在同目录 `events.jsonl` |
| 严格同 workload 单实例 baseline | `20260723-003847`（历史本地制品，未入库） | 100/100，1 个 healthy 实例，吞吐约 1.23 docs/s |
| 严格同 workload 三实例 scaled | `20260723-004244`（历史本地制品，未入库） | workload 指纹一致，100/100，3 个 healthy 实例，吞吐约 4.50 docs/s |
| 三实例 500 文档 | `20260722-233743`（历史本地制品，未入库） | 500/500，三实例各自最大 active 4，原始吞吐约 4.10 docs/s |
| 图片 OCR/caption | `20260723-001833`（历史本地制品，未入库） | OCR/caption/summary/text 均存在，持久 chunk 含 `7319`、`BLUE BOX`、`RED CIRCLE` |
| CSV 表格派生 | `20260723-001858`（历史本地制品，未入库） | table summary/column/summary/text 均存在，持久 chunk 含 `Model`、`StandardReply` |
| XLSX 表格派生 | `20260723-001516`（历史本地制品，未入库） | table summary/column/summary/text 均存在 |
| 普通 PDF | `20260723-001622`（历史本地制品，未入库） | 15 个 text chunk、15 个检索结果 |
| 浏览器排队卡片 | `20260723-002821`（历史本地制品，未入库） | 35 个徽标格式、位置唯一性、全局等待提示和 console error 断言通过 |
| 同实例重启续跑 | `20260722-234526`（历史本地制品，未入库） | 旧 boot 的 4 个文档均在新 boot、epoch 2 上恢复 |
| 跨实例接管 | `20260722-234659`（历史本地制品，未入库） | 4 个被杀 owner 文档逐一 owner 变化且 epoch 递增 |
| pause 安全边界 | `20260722-234952`（历史本地制品，未入库） | pause 约 90 秒，无 owner/epoch 误变化，最终 12/12 |
| Redis 重启恢复 | `20260722-235237`（历史本地制品，未入库） | Redis down/up、认证 PONG、API 恢复、12/12 |
| API 实例重启 | `20260722-235633`（历史本地制品，未入库） | API boot 变化，其他 Worker 文档继续并最终 12/12 |
| 全实例重启 | `20260722-235830`（历史本地制品，未入库） | 三个 boot 全部变化并 ready，最终 12/12 |
...
后续综合验收已经补齐 Word、Excel、PPT、PDF、图片、音频和其他测试集格式，
并在开启向量、摘要、问题生成、知识图谱和 Wiki 的条件下检查完成性。保留的真实
“公司制度”知识库 `efee9ac7-b68b-413e-b09e-77501032a194` 共有 632 份文档：

- 632/632 的 `parse_status`、`summary_status`、`enrichment_status` 和
  `wiki_status` 均为完成，`pending_subtasks=0`。
- PostgreSQL 中有 7,871 个 chunk、13,267 个启用 embedding、5,396 个生成问题。
- 1,660 个已发布 Wiki 页面引用覆盖全部 632 份文档。
- Neo4j 中有 20,008 个节点、13,765 条关系。
- 632 个持久工作流全部完成，没有非终态记录。

另有“数据资料”11/11、“全格式”27/27 完成。该结果是当前
测试环境的可复核基线，不代表所有未来未知/损坏文件都必然成功，也不能外推为
生产 SLA。“公司制度”知识库应保留继续使用，不得为重复专项测试删除或无理由重建。
...
- [ ] 每份文档的成功由 PostgreSQL、向量、Neo4j、Wiki、对象存储和前端联合核对，
      不是只相信 runner 的汇总状态。
- [ ] “公司制度”真实验收知识库保持原样，其他测试失败只重建对应测试知识库。
