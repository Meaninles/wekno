...
- 图片 VLM 理解、扫描 PDF OCR/VLM、音频 ASR 转写。
...
- 音频可使用 Qwen ASR 或其他兼容 ASR。
- 转写/理解结果进入 chunk、向量、摘要和后续衍生。
- VLM/ASR 各自有集群并发，排队不会被标记为跳过。
...
每 app 普通后台 task worker      16
每 app Wiki Map worker           4
```

`asynq.concurrency` 是每 app 的完整文档并发，不是模型总并发。
...
| VLM | 4 | 2 | 图片/页面 |
| ASR | 2 | 1 | 音频 |
| Parser | 12 | 4 | 与 DocReader 对齐 |

DeepSeek V4 Flash 在 32 并发、每次实际生成 160 token 的受控测试中 32/32 成功，
P95 5.655 秒。生产 Chat 取 24，保留约 25% 冗余。模型准入由 Redis 在集群共享，
不能随三个 app 乘三。
...
| 节点 | 规格 | 工作负载 |
|---|---|---|
| `10.14.201.1` | 8C/32Gi + 500G | app、DocReader、两个 Agent、frontend、mobile 各 1 |
| `10.14.201.2` | 8C/32Gi + 500G | 与 `.1` 相同 |
| `10.14.201.7` | 8C/32Gi | app、DocReader 各 1 |
| `10.14.201.6` | 8C/16Gi | 现有 PostgreSQL、LiteLLM、Ingress |
| `10.14.201.54` | 8C/16Gi | 现有 Neo4j、Ingress |

| 组件 | 副本 | request | limit | 临时卷 |
|---|---:|---:|---:|---:|
| app | 3 | 1.5C/3Gi | 4C/8Gi | 80Gi |
| DocReader | 3 | 1.5C/3Gi | 6C/10Gi | 100Gi |
| general-agent | 2 | 0.5C/1Gi | 2C/2Gi | 20Gi |
| document-processing-agent | 2 | 1C/1.5Gi | 3C/4Gi | 40Gi |
...
已经具备多副本：app、DocReader、两个 Agent、frontend、mobile-web。
