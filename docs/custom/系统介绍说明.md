...
生产桌面入口为 `https://knora.moutai.com.cn/`，API 基地址为
`https://knora.moutai.com.cn/api/v1`，移动端为
`https://knora.moutai.com.cn/mobile/`。生产内部节点、Service、对象存储、模型
网关和凭据不在公开文档中记录。
...
| 类型 | 主要用途 |
|---|---|
| 知识问答 | 制度、手册和知识库问答，支持文字总结与改写；越界执行请求直接提示切换相应智能体 |
...
生产模型名称、路由和资源池是受保护的部署值，不在公开说明中硬编码；系统通过
Capacity Control 将每个实际模型路由绑定到唯一资源池，并在每次真实调用前执行集群级
准入。公开可确认的逻辑边界如下：

| 资源类别 | 当前生产语义 |
|---|---|
| 主交互聊天池 | 两个独立池合计最多 48 个执行会话、52 个等待会话，最多接纳 100 个；等待位不跨池借用 |
| 后台/衍生池 | 由 `max_inflight`、`interactive_reserve` 和调度权重计算，空闲时可借用，不能突破资源池总量 |
| Embedding、Rerank、VLM、ASR | 各自使用独立实际模型资源池；单租户、单文档和请求/Token 限额由有效配置统一约束 |
| Web 与 IM | 使用独立的会话 FIFO 计数域，但实际供应商调用仍共享对应模型资源池保护 |

容量页和受保护的生产配置才是具体模型、路由、并发、等待、RPM/TPM 和超时的权威来源；
这些数值不能随 API 或 worker 副本数相乘。
...
| `stateful-services` | PostgreSQL、Redis 与既有集群基础设施，不新增应用 worker |
| `graph-ingress-services` | 现有 Neo4j、Ingress 等入口/图谱服务 |
...
- 业务验收知识库的文档完整状态可通过状态接口与真实召回交叉核对；业务名称和数量不写入版本化文档。
