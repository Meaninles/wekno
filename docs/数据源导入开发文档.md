...
> `https://knora.moutai.com.cn/api/v1`；本文中的本地地址、容器服务名和测试凭据仅用于
> 本地开发/验收，不能复制到生产配置。

WeKnora 的数据源导入模块支持从外部平台自动导入和同步内容到知识库。当前工作区实际
注册并可执行的连接器为：飞书、Notion、语雀和 RSS/Atom。代码中的其他连接器元数据是
预留/展示项，只有完成运行时注册后才能使用，不能仅凭元数据名称宣称已经支持。
...
- WeKnora 已部署并运行
- 已创建至少一个知识库
- 拥有目标外部平台（如飞书、Notion、语雀）的管理权限，或准备可公开访问的 RSS/Atom
  Feed
...
| Notion (`notion`) | `api_key` | Internal Integration Token |
| 语雀 (`yuque`) | `api_token`，可选 `base_url` | API Token |
| RSS / Atom (`rss`) | `settings.feed_urls`，可选 `credentials.auth_headers` | Feed URL；可选自定义请求头 |
...
| POST | `/api/v1/datasource/:id/resource-ancestors` | 补齐懒加载资源选择器所需的祖先资源 |
...
- 连接器类型：`notion`。
- 必填凭证：`credentials.api_key`（Notion Internal Integration Token）。
- 资源：页面和数据库；连接器会读取页面块、数据库内容及可访问的附件。
- 增量同步：按编辑时间和游标处理；删除同步由当前连接器能力标记为不支持。
- 相关实现：`internal/datasource/connector/notion/`。

### 语雀 (Yuque)

- 连接器类型：`yuque`。
- 必填凭证：`credentials.api_token`；可选 `credentials.base_url`，默认使用语雀公有云 API。
- 资源：个人或团队知识库中的已发布文档；草稿和不支持的资源类型会被跳过。
- 增量同步：按文档更新时间和同步游标处理；当前不提供删除同步标记。
- 相关实现：`internal/datasource/connector/yuque/`。

### RSS / Atom

- 连接器类型：`rss`。
- 必填配置：在 `settings.feed_urls` 中填写一个或多个 Feed URL，可换行或逗号分隔。
- 可选认证：在 `credentials.auth_headers` 中填写仅用于 Feed 请求的自定义请求头；不要把
  真实令牌写进文档或日志。
- 增量同步：按 Feed 条目变化和内容指纹处理；Feed 通常不稳定提供删除信号，因此当前不
  进行删除同步。文章链接会按 SSRF 安全策略抓取正文，失败时回退到 Feed 自带内容。
- 相关实现：`internal/datasource/connector/rss/`。
...
| 参数 | 值 | 说明 |
|------|------|------|
| 飞书 Token 缓存安全余量 | 5 min | Token 过期前提前刷新 |
| 飞书 Wiki Space 分页大小 | 50 | ListResources 每页返回数 |
| 飞书导出任务轮询间隔 | 2s | 飞书 Export API 状态检查间隔 |
| 飞书导出任务最大等待 | ~60s | 导出任务超时时间 |
| Notion / 语雀单次 HTTP 请求超时 | 30s | 连接器客户端超时边界 |
| RSS Feed / 文章单次请求超时 | 20s | SSRF 安全客户端的请求超时边界 |
