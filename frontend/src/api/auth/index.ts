...
export interface KnowledgeBaseInfo {
  ...
  creator_id?: string
  // creator_name 由后端 list 接口批量回填（username），
...
export async function getCustomSSOConfig(): Promise<OIDCConfigResponse> {
  try {
    const response = await get('/api/v1/custom/iam/sso/config')
    return response as unknown as OIDCConfigResponse
  } catch (error: any) {
    return {
      success: false,
...
export async function getCustomSSOAuthorizationURL(redirectURI: string): Promise<OIDCAuthURLResponse> {
  try {
    const response = await get(`/api/v1/custom/iam/sso/url?redirect_uri=${encodeURIComponent(redirectURI)}`)
    return response as unknown as OIDCAuthURLResponse
  } catch (error: any) {
    return {
      success: false,
 ...
 * their own username; the token is the authorisation, not an identity
 * lock.
 */
