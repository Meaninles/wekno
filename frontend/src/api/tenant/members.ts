...
// already joined with the user's username/avatar by the backend.
...
/**
 * Invite an existing user (by username) to the tenant with the given role.
 * Backend: POST /api/v1/tenants/:id/members (Owner+).
 *
 * Returns 404 when the username does not match any registered user — the
 * caller should ask the invitee to register first. PR 3 does not yet
 * support invites for users who don't have an account.
 */
