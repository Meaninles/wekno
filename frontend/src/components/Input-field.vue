...
const agentMCPSelectionMode = computed(() => {
  if (!hasAgentConfig.value) return null;
  return currentAgentConfig.value?.mcp_selection_mode || 'all';
});
...
const agentSelectedSkillNames = computed(() => {
  const config = currentAgentConfig.value || {};
  return new Set([
    ...(config.selected_skills || []),
    ...(config.selected_lightweight_skills || []),
    ...(config.selected_professional_skills || []),
  ].map(name => String(name || '').trim()).filter(Boolean));
});

const isSkillAllowedByAgent = (name: string) => {
  if (!hasAgentConfig.value) return true;
  const mode = agentSkillsSelectionMode.value;
  if (mode === 'none') return false;
  if (mode === 'selected') return agentSelectedSkillNames.value.has(name);
  return true;
};
...
const isMCPAllowedByAgent = (service: MCPService) => {
  ...
  if (mode === 'none') return false;
  if (mode === 'selected') {
    const ids = currentAgentConfig.value?.mcp_services || [];
...
const inputPlaceholder = computed(() => {
  ...
  if (hasKnowledge && hasWebSearch) {
    ...
    return t('input.placeholderWithSkill');
...
const getTextareaEl = () => {
  ...
  return el.querySelector('textarea:not([aria-hidden="true"]):not([tabindex="-1"])') || el.querySelector('textarea');
};
...
const handleSend = () => {
  const textareaValue = getTextareaEl()?.value || '';
  const val = query.value || textareaValue;
  if (val !== query.value) {
    query.value = val;
  }
...
const formatAgentNotReadyReasons = (
  ...
  return reasonKeys.map((key) => {
    ...
    if (key === 'database_sources') {
      return t('input.agentMissingDatabaseSources');
    }
    ...
    <div class="rich-input-container" data-guide="chat-input">
      ...
      <div class="control-bar" :class="{ 'is-embedded': embeddedMode }">
        ...
        <div class="control-right">
          ...
          <!-- 发送按钮 -->
          <div v-if="!isReplying" @mousedown.stop.prevent="handleSend" class="control-btn send-btn" data-guide="chat-send"
