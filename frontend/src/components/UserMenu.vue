  ...
  <div class="user-menu" :class="{ 'user-menu--collapsed': uiStore.sidebarCollapsed }" ref="menuRef">
    ...
    <div class="user-button" data-guide="user-menu" @click="toggleMenu">
      ...
      <template v-if="!uiStore.sidebarCollapsed">
        <div class="user-info">
          <!-- 多租户 / superuser：首行租户名，次行 username · 角色。单租户：用户名 + 空间/角色。 -->
          ...
          <template v-else>
            ...
            <div v-if="userSecondaryLine" class="user-secondary">{{ userSecondaryLine }}</div>
          </template>
        </div>
    ...
    <Transition name="dropdown">
      <div v-if="menuVisible" class="user-dropdown" @click.stop>
        ...
        <div class="menu-divider"></div>
        <div ref="chromeExtensionMenuItemRef" class="menu-item menu-item--submenu"
          :class="{ 'is-open': chromeExtensionSubmenuOpen }" @mouseenter="showChromeExtensionSubmenu"
          @mouseleave="scheduleHideChromeExtensionSubmenu">
          <t-icon name="extension" class="menu-icon" />
          <span class="menu-item-label">{{ $t('common.chromeExtension') }}</span>
          <t-icon name="chevron-right" class="menu-chevron" />
        </div>
    ...
    <Teleport to="body">
      <div v-if="chromeExtensionSubmenuOpen" class="chrome-extension-submenu-floating"
        :style="chromeExtensionSubmenuStyle" @mouseenter="showChromeExtensionSubmenu"
        @mouseleave="scheduleHideChromeExtensionSubmenu">
        <button type="button" class="chrome-extension-submenu-item" @click="handleChromeExtensionConfigure">
          <t-icon name="link" class="chrome-extension-submenu-icon" />
          <span>一键配置</span>
        </button>
        <button type="button" class="chrome-extension-submenu-item" @click="handleChromeExtensionDownload">
          <t-icon name="download" class="chrome-extension-submenu-icon" />
          <span>插件下载</span>
        </button>
        <button type="button" class="chrome-extension-submenu-item" @click="handleChromeExtensionGuide">
          <t-icon name="help-circle" class="chrome-extension-submenu-icon" />
          <span>安装指南</span>
        </button>
      </div>
    </Teleport>
...
import {
  ...
  openChromeExtensionGuide,
} from '@/custom/modules/chrome-extension/actions'
...
// 单租户用户（memberships <= 1 且非 superuser）显示为 username + 空间/角色；
// 多租户 / superuser 才显示更完整的租户身份行。Lite 模式下没有 RBAC 概念。
...
const chromeExtensionMenuItemRef = ref<HTMLElement>()
...
const chromeExtensionSubmenuOpen = ref(false)
const chromeExtensionSubmenuStyle = ref<Record<string, string>>({})
...
let chromeExtensionSubmenuHideTimer: ReturnType<typeof setTimeout> | null = null
...
const userSecondaryLine = computed(() => {
  ...
  if (tenantName && tenantName !== userName.value) return tenantName
...
const showIMSubmenu = () => {
  ...
  chromeExtensionSubmenuOpen.value = false
...
const showChromeExtensionSubmenu = () => {
  if (chromeExtensionSubmenuHideTimer) {
    clearTimeout(chromeExtensionSubmenuHideTimer)
    chromeExtensionSubmenuHideTimer = null
  }
  imSubmenuOpen.value = false
  tenantSubmenuOpen.value = false
  positionChromeExtensionSubmenu()
  chromeExtensionSubmenuOpen.value = true
  clampFloatingToViewport('.chrome-extension-submenu-floating', chromeExtensionSubmenuStyle)
}

const scheduleHideChromeExtensionSubmenu = () => {
  if (chromeExtensionSubmenuHideTimer) clearTimeout(chromeExtensionSubmenuHideTimer)
  chromeExtensionSubmenuHideTimer = setTimeout(() => {
    chromeExtensionSubmenuOpen.value = false
    chromeExtensionSubmenuHideTimer = null
  }, 180)
}
...
// Silent prefetch so the live indicator reflects existing IM channel state as
// soon as the user opens the avatar menu. Errors leave the indicator off.
...
const positionChromeExtensionSubmenu = () => {
  const el = chromeExtensionMenuItemRef.value
  if (!el) return
  const zoom = getRootZoom()
  const rect = rectToCssPx(el.getBoundingClientRect(), zoom)
  const { width: vw } = cssViewportSize(zoom)
  ...
  chromeExtensionSubmenuStyle.value = {
    left: `${left}px`,
...
const handleChromeExtensionConfigure = async () => {
  closeAll()
  await oneClickConfigureChromeExtension(router)
}

const handleChromeExtensionDownload = () => {
  closeAll()
  downloadChromeExtensionPackage()
}

const handleChromeExtensionGuide = () => {
  closeAll()
...
const handleClickOutside = (e: MouseEvent) => {
  ...
  const chromeExtensionFloating = document.querySelector('.chrome-extension-submenu-floating')
  if (chromeExtensionFloating && chromeExtensionFloating.contains(target)) return
  menuVisible.value = false
  imSubmenuOpen.value = false
  tenantSubmenuOpen.value = false
  chromeExtensionSubmenuOpen.value = false
...
.chrome-extension-submenu-floating {
  position: fixed;
  z-index: 1100;
  width: 196px;
  padding: 4px;
