                ...
                <t-tooltip :content="item.title" placement="right" :disabled="!uiStore.sidebarCollapsed">
                    <div @click="handleMenuClick(item.path)" @mouseenter="mouseenteMenu(item.path)"
                        ...
                        <div class="menu_item-box">
                            <div class="menu_icon">
                                <img class="icon"
                                    :src="getImgSrc(item.icon == 'zhishiku' ? knowledgeIcon : item.icon == 'agent' ? agentIcon : item.icon == 'data-source' ? dataSourceIcon : item.icon == 'skill' ? skillIcon : item.icon == 'scheduled' ? scheduledIcon : item.icon == 'organization' ? organizationIcon : item.icon == 'logout' ? logoutIcon : item.icon == 'setting' ? settingIcon : prefixIcon)"
                                    alt="">
                            </div>
    ...
    switch (itemPath) {
        case 'knowledge-bases':
            return currentRoute === 'knowledgeBaseList' ||
                ...
                currentRoute === 'dataSourceList';
        case 'agents':
            return currentRoute === 'agentList';
        case 'data-sources':
            return currentRoute === 'dataSourceList';
        case 'skills':
            return currentRoute === 'skillList';
        case 'scheduled-tasks':
            return currentRoute === 'scheduledTaskList';
        case 'organizations':
...
const topMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) =>
        item.path === 'knowledge-bases' || item.path === 'data-sources' || item.path === 'agents' || item.path === 'skills' || item.path === 'scheduled-tasks' || item.path === 'organizations' || item.path === 'creatChat'
    );
});

const bottomMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) => {
        if (item.path === 'knowledge-bases' || item.path === 'data-sources' || item.path === 'agents' || item.path === 'skills' || item.path === 'scheduled-tasks' || item.path === 'organizations' || item.path === 'creatChat') {
            return false;
        }
        return true;
    });
...
let dataSourceIcon = ref('prefixIcon.svg');
let skillIcon = ref('skill.svg');
let scheduledIcon = ref('prefixIcon.svg');
...
const getIcon = (path: string) => {
    ...
    const dataSourcesActiveState = route.name === 'dataSourceList';
    const skillsActiveState = route.name === 'skillList';
    const scheduledActiveState = route.name === 'scheduledTaskList';
    ...
    // 数据源图标：只在数据源页面显示绿色
    dataSourceIcon.value = dataSourcesActiveState ? 'prefixIcon-green.svg' : 'prefixIcon.svg';

    // Skill 图标：只在 Skill 页面显示绿色
    skillIcon.value = skillsActiveState ? 'skill-green.svg' : 'skill.svg';

    // 定时任务图标：只在定时任务页面显示绿色
    scheduledIcon.value = scheduledActiveState ? 'prefixIcon-green.svg' : 'prefixIcon.svg';
...
const handleMenuClick = async (path: string) => {
    if (path === 'knowledge-bases') {
        ...
        router.push('/platform/scheduled-tasks')
