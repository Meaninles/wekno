...
export function shouldSupersedeAgentAnswersForToolCall(dataPayload?: ChatMessage) {
...
export function useChatStreamHandler(options: UseChatStreamHandlerOptions) {
  ...
  const summarizeStreamPayload = (payload: unknown) => {
    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) return undefined
    const record = payload as ChatMessage
  ...
  const reconstructEventStreamFromSteps = (
    ...
    if (agentSteps && Array.isArray(agentSteps) && agentSteps.length > 0) {
      ;(agentSteps as ChatMessage[]).forEach((step) => {
        ...
        if (toolCalls && Array.isArray(toolCalls)) {
          toolCalls.forEach((toolCall: ChatMessage) => {
            ...
            events.push({
              ...
              agent_progress: resultData?.agent_progress
                ? {
                    message: resultData.agent_progress_message,
                    phase: resultData.phase,
                  }
                : undefined,
              agent_progress_history: resultData?.agent_progress
                ? [
                    {
                      message: resultData.agent_progress_message,
                      phase: resultData.phase,
                      updated_at: stepTimestamp || undefined,
                    },
                  ]
                : undefined,
            })
  ...
  const appendAgentErrorEvent = (
    ...
    const existing = stream.find(
      (event) => event.type === 'error' && event.content === errorMsg,
    )
    if (existing) {
      existing.done = true
      return
    ...
    stream.push({
      type: 'error',
      event_id: `agent-error-${Date.now()}`,
      content: errorMsg,
      error: dataPayload?.error || errorMsg,
  ...
  const handleAgentChunk = (data: ChatMessage) => {
    ...
    if (
      ...
      (data.response_type === 'thinking' ||
        ...
        data.response_type === 'agent_progress' ||
    ...
    switch (responseType) {
      ...
      case 'tool_call': {
        ...
        if (dataPayload && (dataPayload.tool_name || dataPayload.tool_call_id)) {
          ...
          if (!toolCallId) {
            console.warn('[Tool Call] Received event without identifiable tool_call_id')
            break
          }
      ...
      case 'agent_progress': {
        ...
        const pending = message._pendingToolCalls as Map<string, ChatMessage>
        const toolCallId =
          (dataPayload?.tool_call_id as string | undefined) ||
          (dataPayload?.progress_id as string | undefined) ||
          'general_agent_progress'
        const progressMessage = String(dataPayload?.message || data.content || '').trim()
        if (!progressMessage) break
        const phase = String(dataPayload?.phase || 'start')
        const transientStatus = dataPayload?.transient === true
        const progressDone = Boolean(data.done) || phase === 'success' || phase === 'error'
        let toolCallEvent = pending.get(toolCallId)
        if (!toolCallEvent) {
          toolCallEvent = stream.find(
            (event) => event.type === 'tool_call' && event.tool_call_id === toolCallId,
        ...
        if (!toolCallEvent) {
          toolCallEvent = {
            ...
            tool_call_id: toolCallId,
            tool_name:
              dataPayload?.tool_name ||
              'general_agent_progress',
            arguments: {},
            timestamp: Date.now(),
            pending: !progressDone,
        ...
        toolCallEvent.tool_name =
          ...
          'general_agent_progress'
        if (transientStatus) {
          toolCallEvent.transient_status = true
        }
        toolCallEvent.pending = !progressDone
        if (progressDone) {
          toolCallEvent.success = phase !== 'error' && dataPayload?.success !== false
          toolCallEvent.output = progressMessage
        }
        const historyKey = 'agent_progress_history'
        const progressKey = 'agent_progress'
        const existingHistory = Array.isArray(toolCallEvent[historyKey])
          ? toolCallEvent[historyKey]
          : []
        const nextProgress = {
        ...
        const progressHistory =
          ...
          existingHistory[existingHistory.length - 1]?.message === progressMessage
            ? existingHistory.slice(0, -1).concat(nextProgress)
            : existingHistory.concat(nextProgress)
        toolCallEvent[historyKey] = progressHistory.slice(-8)
        toolCallEvent[progressKey] = {
          message: progressMessage,
          stage: dataPayload?.stage,
          phase,
          transient: transientStatus,
      ...
      case 'error': {
        if (dataPayload) {
          ...
          if (!toolCallEvent && stream) {
            if (toolCallId) {
              toolCallEvent = stream.find(
                (event) => event.type === 'tool_call' && event.tool_call_id === toolCallId,
              )
            }
            if (!toolCallEvent && toolName) {
              for (let i = stream.length - 1; i >= 0; i--) {
                const event = stream[i]
                if (event.type === 'tool_call' && event.tool_name === toolName) {
                  toolCallEvent = event
                  break
                }
              }
          ...
          if (!toolCallEvent && responseType === 'tool_result' && (toolCallId || toolName)) {
            if (!message.agentEventStream) message.agentEventStream = []
            const fallbackToolCallId =
              toolCallId || (toolName ? `${toolName}_${Date.now()}` : `tool_result_${Date.now()}`)
            toolCallEvent = {
              type: 'tool_call',
              tool_call_id: fallbackToolCallId,
              tool_name: toolName,
              arguments: dataPayload.arguments || {},
              timestamp: Date.now(),
            }
            ;(message.agentEventStream as ChatMessage[]).push(toolCallEvent)
          }
          if (toolCallEvent) {
  ...
  const processStreamChunk = (data: ChatMessage) => {
    log('[Agent Event Received]', {
      ...
      data_summary: summarizeStreamPayload(data.data),
