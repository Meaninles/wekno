...
// KBs, custom agents, and data sources.
//
  ...
  try {
    ...
    return parsed.filter(
      (e: unknown): e is PinEntry =>
        ...
        ((e as PinEntry).type === 'kb' ||
          (e as PinEntry).type === 'agent' ||
          (e as PinEntry).type === 'data_source') &&
...
const favoritesByType: Record<ResourceType, Ref<PinEntry[]>> = {
  kb: ref<PinEntry[]>([]),
  agent: ref<PinEntry[]>([]),
  data_source: ref<PinEntry[]>([]),
}
const loaded: Record<ResourceType, boolean> = { kb: false, agent: false, data_source: false }
const inFlight: Record<ResourceType, Promise<void> | null> = { kb: null, agent: null, data_source: null }
...
function installTenantWatcher(): void {
  ...
  watch(
    ...
    () => {
      loaded.kb = false
      loaded.agent = false
      loaded.data_source = false
      favoritesByType.kb.value = []
      favoritesByType.agent.value = []
      favoritesByType.data_source.value = []
      // Eagerly refetch all types so any already-mounted list view sees
      // fresh data without a manual refresh.
      void fetchFavorites('kb')
      void fetchFavorites('agent')
      void fetchFavorites('data_source')
...
export function useResourcePins(): UseResourcePinsResult {
  ...
  // the in-flight fetch for all types; the templates render with empty
  ...
  if (!loaded.data_source) void fetchFavorites('data_source')

  const favorites = computed<PinEntry[]>(() => {
    // Merge all type lists and sort by ts desc.
    return [
      ...favoritesByType.kb.value,
      ...favoritesByType.agent.value,
      ...favoritesByType.data_source.value,
  ...
  const refresh = async (): Promise<void> => {
    ...
    loaded.agent = false
    loaded.data_source = false
    await Promise.all([fetchFavorites('kb'), fetchFavorites('agent'), fetchFavorites('data_source')])
  }
