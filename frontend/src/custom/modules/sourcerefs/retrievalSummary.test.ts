...
test("only a completed direct text response is classified as a simple conversation", () => {
  ...
  assert.equal(
    isSimpleCompletedConversation({ ...direct, agent_mode: true }),
    false,
  );
  assert.equal(
    isSimpleCompletedConversation({ ...direct, content: "" }),
    false,
  );
  assert.equal(
    isSimpleCompletedConversation({ ...direct, agent_tool_count: 1 }),
    false,
  );
  assert.equal(
    isSimpleCompletedConversation({
  ...
  assert.equal(
    isSimpleCompletedConversation({
      ...direct,
      knowledge_references: [{ id: "cited-source" }],
      retrieval_stats: {
        ...direct.retrieval_stats,
        attempted: true,
        documents: 1,
