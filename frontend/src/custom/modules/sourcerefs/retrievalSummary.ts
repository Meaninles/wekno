...
export function retrievalStatsFromMessage(
...
export function usesDataSourceRetrievalUnit(
  stats: RetrievalStats | null | undefined,
): boolean {
  return Boolean(
    stats && (stats.unit === "data_sources" || stats.dataSources > 0),
  );
}

/** The visible noun and count must describe the same underlying source kind. */
export function retrievalDisplayCount(
  stats: RetrievalStats | null | undefined,
): number {
  if (!stats) return 0;
  if (usesDataSourceRetrievalUnit(stats))
    return Math.max(0, Math.trunc(stats.dataSources));
...
export function isEvidenceRetrievalToolName(toolName: unknown): boolean {
  ...
  return EVIDENCE_RETRIEVAL_TOOLS.some(
    (canonical) =>
      name === canonical ||
      name.endsWith(`__${canonical}`) ||
      name.endsWith(`_${canonical}`),
  );
}

/** Completed summaries use the backend authority so live and reloaded views agree. */
export function agentToolCountFromMessage(
  message: Record<string, any> | undefined,
): number {
  ...
  const stream = Array.isArray(message?.agentEventStream)
    ? message.agentEventStream
    : [];
  return stream.filter(
    (event: any) =>
      event?.type === "tool_call" &&
      !isEvidenceRetrievalToolName(event.tool_name),
  ).length;
}

/** A direct text response has no citation/retrieval/tool telemetry to explain. */
export function isSimpleCompletedConversation(
  message: Record<string, any> | undefined,
): boolean {
  if (message?.is_completed !== true || !String(message?.content || "").trim())
    return false;
  ...
  if (stats?.simpleConversation === true) {
    ...
    return (
      !Array.isArray(message?.knowledge_references) ||
      message.knowledge_references.length === 0
    );
  }
 ...
 */
export function formatCompletedRunDuration(
