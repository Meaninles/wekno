export default {
  menu: {
    informationSources: 'Информационные источники',
  ...
  answerFeedback: {
    ...
    disliked: 'Отмечено как не понравившееся',
  },
  newUserGuide: {
    ...
    kbCreate: {
      steps: {
        ...
        indexing: {
          ...
          desc: 'Стандартный RAG-поиск подходит для большинства вопросов и ответов. Расширенная индексация обычно не нужна; оставьте хотя бы один режим поиска.',
        },
        navModels: {
  ...
  knowledgeBase: {
    ...
    documentQueuePosition: 'Очередь {position}/{total}',
    documentQueuePositionOnly: 'Очередь №{position}',
    documentQueueTooltip: 'Во всей системе ожидают обработки {total} документов; этот документ занимает позицию {position}',
    documentQueueTooltipPositionOnly: 'Этот документ занимает позицию {position} в общей очереди обработки',
    statusFailed: 'Ошибка',
    ...
    filterNoResults: 'Нет документов, соответствующих текущим фильтрам',
    filterNoResultsHint: 'Документы остаются в базе знаний. Измените или сбросьте фильтры.',
    clearDocumentFilters: 'Сбросить фильтры',
    pdfDocFormat: 'Все поддерживаемые форматы при необходимости разделяются автоматически',
    textMarkdownFormat: 'До 2 ГБ на логический документ (настраивается при развёртывании)',
  ...
  knowledgeWorkflowStatus: {
    title: 'Полный статус обработки',
    coreReadyEnriching: 'Основное содержимое доступно, идёт обогащение',
    unknown: 'Статус неизвестен',
    detailsUnavailable: 'Детали этапов временно недоступны',
    hoverHint: 'Актуальные этапы загружаются при наведении без постоянного опроса',
    status: {
      pending: 'В очереди',
      processing: 'Обработка',
      cancelling: 'Отмена',
      ...
      draft: 'Черновик',
      unknown: 'Статус неизвестен',
      not_started: 'Ожидание предыдущего этапа',
      disabled: 'Не включено',
      not_applicable: 'Неприменимо',
      skipped: 'Не требуется',
      blocked: 'Ошибка предыдущего этапа',
    },
    stage: {
      parse: 'Разбор и разбиение',
      embedding: 'Векторизация',
      multimodal: 'Обработка изображений / аудио',
      summary: 'Сводка',
  ...
  knowledgeStages: {
    ...
    attempt: 'Запуск обработки {n}',
    attemptLatest: 'Запуск обработки {n} (последний)',
    attemptHelp: 'Загрузка или ручной повторный разбор создаёт новый запуск; автоматические повторы задач остаются в том же запуске.',
  ...
  agent: {
    ...
    editor: {
      ...
      nativeAgentWebSearch: 'Встроенный поиск универсального агента',
...
/example\.(net|org)/`,
  ...
  auth: {
    ...
    passwordPlaceholder: 'Введите пароль (8-32 символа, две категории из букв/цифр/символов)',
    ...
    passwordRuleHint: 'Правило пароля: 8–32 символа, минимум две категории из букв, цифр и символов, без пробелов.',
    passwordNoWhitespace: 'Пароль не должен содержать пробелы',
    passwordMustUseTwoTypes: 'Пароль должен содержать минимум две категории из букв, цифр и символов',
  ...
  system: {
    ...
    globalSettings: {
      ...
      keyLabels: {
        ...
        asynq: {
          concurrency: 'Параллелизм документов на экземпляр',
          heavy_document_concurrency: 'Параллелизм тяжёлых документов (расширенный)',
        },
      },
      keyDescriptions: {
        auth: {
        ...
        asynq: {
          concurrency:
            'Число документов, которые каждый полнофункциональный экземпляр может обрабатывать одновременно. Один слот сопровождает документ через разбор, разбиение, векторизацию и связанные производные задачи; добавление экземпляров горизонтально увеличивает общую пропускную способность. После изменения перезапустите все экземпляры обработки.',
          heavy_document_concurrency:
            'Защитный предел на экземпляр для документов, превысивших пороги размера или структурной сложности. Для обычного планирования используйте «Параллелизм документов на экземпляр»; этот расширенный параметр редко требует изменения и применяется после перезапуска всех экземпляров.',
        },
      },
      enumLabels: {
        auth: {
  ...
  model: {
    ...
    editor: {
      ...
      description: {
        ...
        derivative: 'Настройте выделенную модель для Wiki, графов, сводок и других фоновых задач',
      ...
      asrResponseFormatLabel: 'Формат ответа ASR',
      asrResponseFormatDesc:
        'response_format для /v1/audio/transcriptions. По умолчанию verbose_json; некоторые совместимые сервисы поддерживают только json.',
  ...
  knowledgeEditor: {
    ...
    basic: {
      ...
      typeDescription: 'FAQ подходит для структурированных Q&A; документальный тип поддерживает разбор файлов, разбиение и поиск.',
    ...
    indexing: {
      ...
      description: 'RAG-поиск подходит для большинства вопросов и ответов. Раскрывайте расширенные параметры только при необходимости более дорогого индекса.',
      ...
      advancedTitle: 'Расширенная индексация',
      advancedDesc: 'Требует дополнительных вызовов модели и обычно не нужна',
      advancedEnabledSummary: 'Wiki включена · {granularity}',
      ...
      wikiCostDesc: 'Wiki вызывает модель для создания и постоянного обновления связанных страниц, поэтому стоимость растёт с числом документов. Включайте её только для чтения по каталогу или организации знаний между документами; для обычных вопросов используйте RAG.',
      wikiUnavailable: 'Wiki временно недоступна. Обратитесь к администратору, чтобы включить доступ к Wiki.',
    ...
    wikiBrowser: {
      ...
      graphNodeCatalogue: 'Категории узлов',
      graphSearchNodes: 'Поиск по всем узлам',
      graphNoMatchingNodes: 'Подходящих узлов нет',
      graphSelectNodeHint: 'Выберите узел в списке категорий слева',
      graphBackCenter: 'Предыдущий узел',
      graphViewDetails: 'Подробнее',
      graphSetAsCenter: 'Сделать центром',
      graphNeighborProgress: 'На странице {returned} / всего {total} связанных узлов',
      graphHelpListAction: 'Выбрать узел списка',
      graphHelpListDesc: 'Показать связи первого уровня',
      graphHelpNeighborAction: 'Клик по связанному узлу',
      graphHelpNeighborDesc: 'Сделать этот узел центром',
      graphHelpCenterAction: 'Клик по центру',
      graphHelpCenterDesc: 'Открыть сведения об узле',
    ...
    advanced: {
      ...
      questionGeneration: {
        ...
        countDescription: 'Количество вопросов на фрагмент документа (1-3, по умолчанию 1)'
      },
      multimodal: {
  ...
  modelSettings: {
    ...
    derivative: {
      title: 'Модели производных задач',
      desc: 'Модели только для фоновых производных задач',
      empty: 'Нет моделей производных задач'
    },
    toasts: {
  ...
  input: {
    ...
    placeholderWithSkill: 'Введите вопрос, ответ будет учитывать выбранный Skill',
    ...
    agentMissingDatabaseSources: 'Источники данных базы данных',
    ...
    notConfigured: 'Не настроено',
    agentModelLabel: 'Модель из агента',
    ...
    messages: {
      ...
      fileInfoLoading: 'Данные выбранного файла ещё загружаются. Повторите попытку через несколько секунд.',
  ...
  skill: {
    title: 'Skills',
    subtitle: 'Управляйте легкими ограничениями и рабочими процессами для агентов или прямого контекста чата.',
    create: 'Новый Skill',
    noDescription: 'Нет описания',
    enabled: 'Включен',
    disabled: 'Отключен',
    deleteConfirm: 'Удалить Skill "{name}"? Действие необратимо.',
    origin: {
      mine: 'Мои',
      space: 'Общее пространство',
      userShared: 'Поделился коллега',
    },
    empty: {
      title: 'Skills пока нет',
      desc: 'Создайте Skill для повторяющихся процессов, тона или формата ответа.',
    },
    selector: {
      searchPlaceholder: 'Поиск Skill',
      professionalTab: 'Проф.',
      lightweightTab: 'Легкие',
      empty: 'Нет доступных Skills',
      professionalEmpty: 'Нет доступных профессиональных Skills',
      noMatch: 'Нет совпадений',
      sandboxUnavailable: 'Skills не включены в этой среде',
      professionalUnavailable: 'Профессиональные Skills не включены в этой среде',
      manage: 'Управлять',
    },
    editor: {
      createTitle: 'Новый Skill',
      editTitle: 'Редактировать Skill',
      viewTitle: 'Просмотр Skill',
      enabled: 'Включен',
      skillMd: 'SKILL.md',
      templateTip: 'Следуйте формату SKILL.md: name и description в YAML frontmatter, затем сценарии, процесс и требования к выводу в Markdown.',
      readOnlyTip: 'Этот Skill доступен вам через общий доступ. Его можно просматривать, но редактировать и сохранять может только владелец или администратор.',
      invalidFrontmatter: 'Сохраните YAML frontmatter, обернутый в ---',
      requiredFields: 'name, description и тело обязательны',
    },
    share: {
      title: 'Поделиться Skill',
      action: 'Поделиться',
      spaceTab: 'Пространство',
      userTab: 'Пользователь',
      sharedSpaces: 'Общие пространства',
      sharedUsers: 'Пользователи',
      noUserShares: 'Нет пользовательских шар',
      userSearchPlaceholder: 'Поиск пользователя',
      userShareSuccess: 'Поделились с пользователем',
      userShareFailed: 'Не удалось поделиться с пользователем',
      loadFailed: 'Не удалось загрузить список шар',
      searchUserFailed: 'Не удалось найти пользователей',
    },
    messages: {
      loadFailed: 'Не удалось загрузить Skills',
      created: 'Skill создан',
      updated: 'Skill обновлен',
      deleted: 'Skill удален',
      saveFailed: 'Не удалось сохранить Skill',
    },
  },
  preview: {
    tab: 'Предпросмотр',
    ...
    safePreviewTitle: 'Включен безопасный просмотр',
    safePreviewHint: 'Документ слишком большой или имеет сложную структуру, поэтому исходный файл не загружается в память браузера целиком. Используйте постраничное распознанное содержимое.',
    viewPagedContent: 'Открыть постраничное содержимое',
    truncatedHint: 'Для сохранения отзывчивости показаны только первые 512 КБ. Полное содержимое доступно в режиме фрагментов.',
    audioSafePreviewHint: 'Аудиофайл слишком большой для просмотра в памяти браузера; полная загрузка остановлена.',
  },
  commandPalette: {
  ...
  agentStream: {
    ...
    tools: {
      ...
      tableAnalysis: 'Анализ таблиц',
      tableSchema: 'Структура таблицы',
      tableAnalysisDisplayIntent: 'Намерение диаграммы',
      databaseQuery: 'Запрос к базе данных',
      dbCatalog: 'Каталог базы данных',
      dbSchema: 'Схема базы данных',
      dbQuery: 'Анализ базы данных'
    },
    summary: {
  ...
  agentEditor: {
    ...
    resetBuiltin: {
      ...
      confirmBody: 'Встроенный агент будет восстановлен к системным настройкам по умолчанию. Текущие модель, источники данных и MCP сохранятся.',
      confirmOk: 'Восстановить',
      success: 'Конфигурация по умолчанию восстановлена',
      failed: 'Не удалось восстановить конфигурацию по умолчанию',
    },
    navGroups: {
      basic: 'Основное',
    ...
    desc: {
      ...
      nativeAgentWebSearch: 'Только для анализа данных, универсального агента и обработки документов. Если включено, соответствующая универсальная среда получает встроенные WebSearch/WebFetch; если выключено, эти инструменты не включаются.',
    ...
    dataAnalysis: {
      enableLabel: 'Включить устаревший анализ данных',
