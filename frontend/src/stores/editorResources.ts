...
const emptyPlaceholders = (): PlaceholdersResponse => ({
  all: [],
  system_prompt: [],
  agent_system_prompt: [],
  context_template: [],
...
export const useEditorResourcesStore = defineStore('editorResources', () => {
  ...
  const placeholders = ref<PlaceholdersResponse>(emptyPlaceholders())
  ...
  async function ensureSkills(force = false): Promise<void> {
    return runOnce('skills', force, async () => {
      try {
        ...
        professionalSkillsAvailable.value = skillsRes.professional_skills_available === true
        professionalSkills.value = skillsRes.professional_data && skillsRes.professional_data.length > 0 ? skillsRes.professional_data : []
      } catch {
  ...
  async function ensurePlaceholders(force = false): Promise<void> {
    return runOnce('placeholders', force, async () => {
      ...
      placeholders.value = placeholdersRes?.data ?? emptyPlaceholders()
  ...
  function invalidate(...keys: EditorResourceKey[]) {
    if (keys.length === 0) {
      ...
      skills.value = []
      professionalSkills.value = []
      skillsAvailable.value = true
      professionalSkillsAvailable.value = false
      agentTypePresets.value = []
      promptTemplates.value = null
      placeholders.value = emptyPlaceholders()
