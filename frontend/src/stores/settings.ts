...
export const useSettingsStore = defineStore("settings", {
  ...
  actions: {
    ...
    // 选择 Skills（替换整个列表）
    selectSkillNames(skillNames: string[]) {
    ...
    addSkillName(skillName: string) {
      const name = String(skillName || "").trim();
      if (!name) return;
      if (!this.settings.selectedSkillNames) this.settings.selectedSkillNames = [];
      if (!this.settings.selectedSkillNames.includes(name)) {
        this.settings.selectedSkillNames.push(name);
        localStorage.setItem("WeKnora_settings", JSON.stringify(this.settings));
      }
    },

    removeSkillName(skillName: string) {
      if (!this.settings.selectedSkillNames) return;
      this.settings.selectedSkillNames =
        this.settings.selectedSkillNames.filter((name: string) => name !== skillName);
      localStorage.setItem("WeKnora_settings", JSON.stringify(this.settings));
    },
