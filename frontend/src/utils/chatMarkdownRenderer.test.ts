...
test('renderChatMarkdown drops invalid markdown image placeholders', () => {
  const renderer = createChatMarkdownRenderer({
    imageRenderer: ({ href, text }) => `<img src="${href}" alt="${text}">`,
    invalidImageHtml: () => '',
    isValidImageUrl: (href) => href.startsWith('https://') || href.startsWith('/'),
  })
  const html = renderChatMarkdown('![商品品类占比](各品类的消费占比)\n\n![ok](https://example.com/chart.png)', {
    renderer,
    escapeMarkdown: (text) => text,
    sanitizeHtml: (html) => html,
  })
