...
const PUBLIC_AUTH_PATHS = ['/auth/auto-setup', '/auth/login', '/auth/register', '/auth/oidc/', '/custom/iam/sso/', '/auth/invitations/lookup', '/api/v1/embed/'];
...
async function extractErrorMessage(data: any): Promise<string | undefined> {
  if (typeof Blob !== 'undefined' && data instanceof Blob) {
    try {
      ...
      try {
        return await extractErrorMessage(JSON.parse(text));
      } catch {
        return text;
      }
    } catch {
...
instance.interceptors.response.use(
  ...
  async (error: any) => {
    ...
    let errorMessage: string | undefined = await extractErrorMessage(data);
    return Promise.reject({ 
      status, 
...
export async function getDown(url: string, config: AxiosRequestConfig = {}): Promise<Blob> {
