          ...
          <div class="settings-container">
            ...
            <div class="settings-content">
              <div ref="contentWrapperRef" class="content-wrapper" :class="{ 'content-wrapper--prompts': currentSection === 'prompts' }">
                ...
                <!-- 文档模板（仅文档处理智能体） -->
                <div v-if="isDocumentProcessingAgent" v-show="currentSection === 'document-template'" class="section">
                  <div class="section-header">
                    <h2>文档模板</h2>
                    <p class="section-description">为 Word、Excel、PDF、PPT 分别配置模板要求文件和参考文档。模板要求是格式要求，参考文档是软模板。</p>
                  </div>

                  <div class="settings-group document-template-settings">
                    <div v-for="item in documentTemplateFormats" :key="item.key"
                      class="setting-row setting-row-vertical document-template-format">
                      <div class="setting-info full-width">
                        <div class="setting-info-header">
                          <label>{{ item.label }}</label>
                          <t-tag size="small" variant="light">{{ item.outputHint }}</t-tag>
                        </div>
                        <p class="desc">{{ item.description }}</p>
                      </div>

                      <div class="document-template-grid">
                        <div class="document-template-panel">
                          <div class="document-template-panel__header">
                            <div>
                              <div class="document-template-panel__title">模板要求文件</div>
                              <div class="document-template-panel__desc">最多 1 份，仅支持 Markdown / TXT。</div>
                            </div>
                            <div class="document-template-panel__actions">
                              <t-button size="small" variant="outline"
                                :disabled="props.readOnly || isDocumentTemplateUploading(item.key, 'requirement')"
                                :loading="isDocumentTemplateUploading(item.key, 'requirement')"
                                @click="chooseRequirementFile(item.key)">
                                上传
                              </t-button>
                              <t-button size="small" variant="text" :disabled="props.readOnly"
                                @click="restoreDefaultRequirementFile(item.key)">
                                恢复默认
                              </t-button>
                            </div>
                          </div>

                          <div v-if="documentTemplateUploadItemsFor(item.key, 'requirement').length"
                            class="document-template-upload-list">
                            <div v-for="upload in documentTemplateUploadItemsFor(item.key, 'requirement')"
                              :key="upload.id" class="document-template-upload-item"
                              :class="`document-template-upload-item--${upload.status}`">
                              <div class="document-template-upload-item__main">
                                <t-loading v-if="upload.status === 'reading'" size="small" />
                                <t-icon v-else :name="upload.status === 'error' ? 'close-circle' : 'check-circle'" />
                                <div class="document-template-file__text">
                                  <div class="document-template-file__name">{{ upload.file_name }}</div>
                                  <div class="document-template-file__meta">
                                    {{ documentTemplateUploadMeta(upload) }}
                                  </div>
                                </div>
                              </div>
                              <t-button v-if="upload.status === 'error'" size="small" variant="text"
                                :disabled="props.readOnly" @click="removeDocumentTemplateUploadItem(upload.id)">
                                移除
                              </t-button>
                            </div>
                          </div>

                          <div v-if="getDocumentTemplateFormat(item.key).requirement_file"
                            class="document-template-file">
                            <div class="document-template-file__main">
                              <t-icon name="file" />
                              <div class="document-template-file__text">
                                <div class="document-template-file__name">
                                  {{ getDocumentTemplateFormat(item.key).requirement_file?.file_name }}
                                </div>
                                <div class="document-template-file__meta">
                                  {{ documentTemplateFileMeta(getDocumentTemplateFormat(item.key).requirement_file) }}
                                </div>
                              </div>
                            </div>
                            <t-button size="small" variant="text" theme="danger" :disabled="props.readOnly"
                              @click="removeRequirementFile(item.key)">
                              删除
                            </t-button>
                          </div>
                          <div v-else class="document-template-empty">未配置模板要求文件；运行时会使用提示词中的通用兜底要求。</div>
                        </div>

                        <div class="document-template-panel">
                          <div class="document-template-panel__header">
                            <div>
                              <div class="document-template-panel__title">参考文档</div>
                              <div class="document-template-panel__desc">最多 {{ documentTemplateReferenceLimit(item.key) }} 份，{{ item.referenceHint }}。仅作为软模板参考。</div>
                            </div>
                            <t-button size="small" variant="outline"
                              :disabled="props.readOnly || isDocumentTemplateUploading(item.key, 'reference')"
                              :loading="isDocumentTemplateUploading(item.key, 'reference')"
                              @click="chooseReferenceFiles(item.key)">
                              上传
                            </t-button>
                          </div>

                          <div v-if="documentTemplateUploadItemsFor(item.key, 'reference').length"
                            class="document-template-upload-list">
                            <div v-for="upload in documentTemplateUploadItemsFor(item.key, 'reference')"
                              :key="upload.id" class="document-template-upload-item"
                              :class="`document-template-upload-item--${upload.status}`">
                              <div class="document-template-upload-item__main">
                                <t-loading v-if="upload.status === 'reading'" size="small" />
                          ...
                          <div v-if="getDocumentTemplateFormat(item.key).template_files?.length"
                            class="document-template-file-list">
                            <div v-for="(file, index) in getDocumentTemplateFormat(item.key).template_files"
                              :key="`${file.file_name}-${index}`" class="document-template-file">
                              <div class="document-template-file__main">
                                <t-icon name="file" />
                                <div class="document-template-file__text">
                                  <div class="document-template-file__name">{{ file.file_name }}</div>
                                  <div class="document-template-file__meta">{{ documentTemplateFileMeta(file) }}</div>
                                </div>
                              </div>
                              <t-button size="small" variant="text" theme="danger" :disabled="props.readOnly"
                                @click="removeReferenceFile(item.key, index)">
                                删除
                              </t-button>
                            </div>
                          </div>
                          <div v-else class="document-template-empty">未配置参考文档；运行时只使用模板要求和通用兜底要求。</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div v-show="currentSection === 'model'" class="section">
                  ...
                  <div class="settings-group">
                    ...
                    <div class="setting-row">
                      ...
                      <div class="setting-control">
                        <span>{{ iterationBudget }} 次（固定）</span>
                      </div>
                    </div>
                    ...
                    <div class="setting-row">
                      ...
                      <div class="setting-control">
                        <t-input-number v-model="formData.config.llm_call_timeout" :min="0"
                          :max="AGENT_LLM_TIMEOUT_LIMIT_SECONDS" theme="column"
                          :placeholder="$t('agentEditor.llmCallTimeout.placeholder')" clearable />
                      </div>
                    </div>

                    <!-- 产物设置 -->
                    <template v-if="isGeneralRuntimeAgent">
                      <div class="setting-row">
                        <div class="setting-info">
                          <label>允许生成文件</label>
                          <p v-if="isKnowledgeBaseManager" class="desc">知识库管理必须开启产物能力：联网内容需先生成并注册为本轮产物，再交给知识库原生解析。</p>
                          <p v-else class="desc">开启后，智能体可以按用户需求生成可下载文件；单次回答的文件合计不超过 128 MiB，超限会明确提示。</p>
                        </div>
                        <div class="setting-control">
                ...
                <div v-show="currentSection === 'tools' && isAgentMode" class="section">
                  ...
                  <div class="settings-group">
                    ...
                    <div
                      class="setting-row setting-row-vertical"
                      data-agent-field="database_sources"
                      :class="{ 'setting-row--field-highlight': highlightedField === 'database_sources' }"
                    >
                      <div class="setting-info">
                  ...
                  <div class="settings-group">
                    <div class="skill-type-tabs">
                      <button type="button" class="skill-type-tab"
                        :class="{ active: skillsActiveTab === 'lightweight' }"
                    ...
                    <div v-show="skillsActiveTab === 'lightweight'">
                    <!-- Skills 选择模式 -->
                    <div class="setting-row">
                      <div class="setting-info">
                        <label>轻量技能选择</label>
                        <p class="desc">轻量技能会作为预置提示词注入上下文；对话页面也只能临时选择轻量技能。</p>
                      </div>
                      <div class="setting-control">
                    ...
                    <div v-if="skillsSelectionMode === 'selected' && skillOptions.length > 0"
                      ...
                      <div class="setting-info">
                        <label>选择轻量技能</label>
                        <p class="desc">可与专业技能混搭；这些内容会直接进入提示词上下文。</p>
                      </div>
                      <div class="setting-control setting-control-full">
                        <t-checkbox-group v-model="formData.config.selected_lightweight_skills" class="skills-checkbox-group">
                    ...
                    <div v-show="skillsActiveTab === 'professional' && isGeneralRuntimeAgent">
                      ...
                      <div v-if="professionalSkillsSelectionMode === 'selected' && professionalSkillOptions.length > 0"
                        class="setting-row setting-row-vertical">
                        <div class="setting-info">
                          <label>选择专业技能</label>
                          <p class="desc">可选择多个专业技能；运行时会写入本次通用智能体工作目录并按配置加载。</p>
                        </div>
                        <div class="setting-control setting-control-full">
                          <t-checkbox-group v-model="formData.config.selected_professional_skills" class="skills-checkbox-group">
                            <t-checkbox v-for="skill in professionalSkillOptions" :key="skill.name" :value="skill.name"
                              class="skill-checkbox-item">
                              <div class="skill-item-content">
                                <span class="skill-name">{{ skill.name }}</span>
                                <span class="skill-desc">{{ skill.description }}</span>
                      ...
                      <div v-if="professionalSkillOptions.length === 0" class="setting-row">
                        <div class="setting-info">
                          <p class="desc empty-hint">暂无专业技能。</p>
                        </div>
                      </div>
                    </div>
                ...
                <div v-show="currentSection === 'database' && canUseDatabaseSources" class="section">
                  <div class="section-header">
                    <h2>数据源</h2>
                    <p class="section-description">选择这个智能体可以访问的 MySQL / PostgreSQL 数据源。</p>
                  </div>
                  ...
                  <div class="settings-group">
                    <div class="setting-row setting-row-vertical">
                      <div class="setting-info">
                        <label>绑定数据源</label>
                        <p class="desc">运行时只会注册数据库目录、结构和查询工具访问这里选中的数据源。</p>
                      </div>
                      <div class="setting-control setting-control-full">
                        <t-select v-model="formData.config.db_data_sources" multiple filterable
                          :min-collapsed-num="3" placeholder="选择数据源">
                          <t-option v-for="source in dbSourceOptions" :key="source.id" :value="source.id"
                            :label="source.name" :disabled="source.status !== 'active'">
                            <div class="db-source-option">
                              <span class="db-source-option__name">{{ source.name }}</span>
                              <span class="db-source-option__meta">{{ source.type }} · {{ source.config?.database ||
                                source.query_mode }}</span>
                              <t-tag v-if="source.status !== 'active'" size="small" theme="danger" variant="light">异常</t-tag>
                            </div>
                          </t-option>
                        </t-select>
                        <div class="db-source-actions">
                          <t-button size="small" variant="outline" @click="openDataSourcePage">
                            管理数据源
                          </t-button>
                          <span v-if="dbSourceOptions.length === 0" class="desc">还没有可用数据源，请先创建并刷新元数据。</span>
                        </div>
                      </div>
                    </div>
                  </div>
...
import { listDatabaseSources, type DatabaseSource } from '@/api/dbanalytics';
...
const VALID_HIGHLIGHT_FIELDS: AgentNotReadyReasonKey[] = ['summary_model', 'rerank_model', 'allowed_tools', 'database_sources'];

const sectionForHighlightField = (field: AgentNotReadyReasonKey): string => {
  if (field === 'allowed_tools') return 'tools';
  if (field === 'database_sources') return 'database';
  return 'model';
};
...
const selectedModelOwnsSampling = computed(() => selectedModelExtra.value.generation_policy === 'gateway');
...
// Agent (agent) 模式的默认系统提示词。直接从 prompt-templates
...
// Wiki 读取类工具（用于 watch(agentMode) 切到 agent 时 seed 默认工具）
...
const professionalSkillsSelectionMode = ref<'all' | 'selected' | 'none'>('none');
const skillsActiveTab = ref<'lightweight' | 'professional'>('lightweight');
...
//   2) 无论是否勾选，web_search 随 web_search_enabled 出现；web_fetch 还需要 web_fetch_enabled
...
const documentTemplatePlaceholderNames = new Set([
...
const availablePlaceholders = computed(() => {
  const items = placeholderData.value.agent_system_prompt;
  if (isDocumentProcessingAgent.value) return items;
  return items.filter((item) => !documentTemplatePlaceholderNames.has(item.name));
});
...
const navItems = computed(() => {
  ...
  if (isDocumentProcessingAgent.value) {
    items.push({ key: 'document-template', icon: 'file', label: '文档模板' });
  }
  items.push({ key: 'model', icon: 'control-platform', label: t('agent.editor.modelConfig') });
  // 多轮对话（仅普通模式显示，Agent模式内部自动控制）
  ...
  return [
    {
      ...
      items: pickItems(['basic', 'prompts', 'document-template', 'model', 'conversation']),
    },
    {
      key: 'knowledge',
      label: t('agentEditor.navGroups.knowledge'),
      items: pickItems(['knowledge', 'database', 'retrieval', 'websearch']),
    },
    {
      key: 'capability',
...
const AGENT_LLM_TIMEOUT_LIMIT_SECONDS = 600;
const DOCUMENT_TEMPLATE_DEFAULT_MAX_FILE_SIZE_BYTES = 20 * 1024 * 1024;
const DOCUMENT_TEMPLATE_PPT_REFERENCE_MAX_FILE_SIZE_BYTES = 100 * 1024 * 1024;
const DOCUMENT_TEMPLATE_DEFAULT_REFERENCE_LIMIT = 3;
const DOCUMENT_TEMPLATE_PPT_REFERENCE_LIMIT = 3;

type DocumentTemplateFormatKey = 'word' | 'excel' | 'pdf' | 'ppt';
type DocumentTemplateFileRole = 'requirement' | 'reference';
type DocumentTemplateUploadStatus = 'reading' | 'done' | 'error';

interface DocumentTemplateUploadItem {
  id: string;
  format: DocumentTemplateFormatKey;
  role: DocumentTemplateFileRole;
  file_name: string;
  file_type: string;
  file_size: number;
  status: DocumentTemplateUploadStatus;
  message: string;
}

let documentTemplateUploadSeq = 0;
const documentTemplateUploadItems = ref<DocumentTemplateUploadItem[]>([]);

const documentTemplateDefaults: Record<DocumentTemplateFormatKey, DocumentTemplateFile> = {
  word: {
    id: 'gbt9704_2012_word_requirement',
    source: 'builtin',
  ...
  excel: {
    id: 'gbt9704_2012_excel_requirement',
    source: 'builtin',
  ...
  pdf: {
    id: 'gbt9704_2012_pdf_requirement',
    source: 'builtin',
  ...
  ppt: {
    id: 'gbt9704_2012_ppt_requirement',
    source: 'builtin',
...
const createDefaultDocumentTemplateConfig = (): DocumentTemplateConfig => ({
  word: { requirement_file: { ...documentTemplateDefaults.word }, template_files: [] },
  excel: { requirement_file: { ...documentTemplateDefaults.excel }, template_files: [] },
  pdf: { requirement_file: { ...documentTemplateDefaults.pdf }, template_files: [] },
  ppt: { requirement_file: { ...documentTemplateDefaults.ppt }, template_files: [] },
});
...
const defaultFormData = {
  ...
  config: {
    // 基础设置
    agent_mode: 'agent' as const,
    system_prompt: '',
    document_template: createDefaultDocumentTemplateConfig(),
    // 模型设置
    model_id: '',
    rerank_model_id: '',
    temperature: 0.7,
    ...
    lightweight_skills_selection_mode: 'none' as 'all' | 'selected' | 'none',
    selected_lightweight_skills: [] as string[],
    professional_skills_selection_mode: 'none' as 'all' | 'selected' | 'none',
    selected_professional_skills: [] as string[],
    ...
    // 知识问答下的类型预设：新建 agent 时默认给 RAG 问答（最常用场景）。
...
const agentMode = computed({
  ...
  set: (val: 'agent') => { formData.value.config.agent_mode = val; }
});
...
// ============================================================================
// 智能体类型预设（仅 agent 模式下可见）
// 选择类型后自动填充 system_prompt_id / allowed_tools 等；
// 选择 "custom" 或没有匹配预设时不做任何覆盖。
// ============================================================================

const agentType = computed({
  get: () => (formData.value.config.agent_type as AgentType) || 'general-agent',
  set: (val: AgentType) => { formData.value.config.agent_type = val; },
});

const isDataAnalysisAgent = computed(() => agentType.value === 'data-analysis');
const isTableAnalysisAgent = computed(() => agentType.value === 'table-analysis');
const isFixedAnalysisAgent = computed(() => isDataAnalysisAgent.value || isTableAnalysisAgent.value);
const isGeneralAgent = computed(() => agentType.value === 'general-agent');
...
const documentTemplateFormats = computed(() => [
  {
    key: 'word' as DocumentTemplateFormatKey,
    label: 'Word',
    outputHint: '.doc / .docx',
    description: '用于正式公文正文、通知、请示、报告、函、纪要等 Word 文档的新建与规范化。',
    referenceHint: '支持 .doc / .docx',
  },
  {
    key: 'excel' as DocumentTemplateFormatKey,
    label: 'Excel',
    outputHint: '.xls / .xlsx',
    description: '用于公文附件、统计表、清单、台账、汇总表等 Excel 文档的新建与规范化。',
    referenceHint: '支持 .xls / .xlsx',
  },
  {
    key: 'pdf' as DocumentTemplateFormatKey,
    label: 'PDF',
    outputHint: '.pdf',
    description: '用于最终发布、归档、打印或发送的 PDF 公文及 PDF 版附件。',
    referenceHint: '支持 .pdf',
  },
  {
    key: 'ppt' as DocumentTemplateFormatKey,
    label: 'PPT',
    outputHint: '.ppt / .pptx',
    description: '用于汇报、述职、分析材料、演示文稿等 PPT 文档的新建与规范化。',
    referenceHint: '支持 .ppt / .pptx，单个文件最高 100MB',
  },
]);

const documentTemplateReferenceLimit = (format: DocumentTemplateFormatKey): number =>
  format === 'ppt' ? DOCUMENT_TEMPLATE_PPT_REFERENCE_LIMIT : DOCUMENT_TEMPLATE_DEFAULT_REFERENCE_LIMIT;

const documentTemplateFileSizeLimit = (format: DocumentTemplateFormatKey, role: DocumentTemplateFileRole): number =>
  format === 'ppt' && role === 'reference'
    ? DOCUMENT_TEMPLATE_PPT_REFERENCE_MAX_FILE_SIZE_BYTES
    : DOCUMENT_TEMPLATE_DEFAULT_MAX_FILE_SIZE_BYTES;

const documentTemplateUploadItemsFor = (format: DocumentTemplateFormatKey, role: DocumentTemplateFileRole) =>
  documentTemplateUploadItems.value.filter(item => item.format === format && item.role === role);

const isDocumentTemplateUploading = (format: DocumentTemplateFormatKey, role: DocumentTemplateFileRole): boolean =>
  documentTemplateUploadItems.value.some(item => item.format === format && item.role === role && item.status === 'reading');

const documentTemplateUploadMeta = (item: DocumentTemplateUploadItem): string => {
  const ext = (item.file_type || '').replace(/^\./, '').toUpperCase() || 'FILE';
  return [item.message, ext, formatBytes(item.file_size)].filter(Boolean).join(' · ');
};

const addDocumentTemplateUploadItem = (
  format: DocumentTemplateFormatKey,
  role: DocumentTemplateFileRole,
  file: File,
): string => {
  const id = `${format}-${role}-${Date.now()}-${documentTemplateUploadSeq++}`;
  documentTemplateUploadItems.value.push({
    id,
    format,
    role,
    file_name: file.name,
    file_type: getFileExt(file.name),
    file_size: file.size,
    status: 'reading',
    message: '正在读取文件',
  });
...
const updateDocumentTemplateUploadItem = (
  id: string,
  patch: Partial<Pick<DocumentTemplateUploadItem, 'status' | 'message'>>,
) => {
  const index = documentTemplateUploadItems.value.findIndex(item => item.id === id);
  if (index < 0) return;
  documentTemplateUploadItems.value[index] = { ...documentTemplateUploadItems.value[index], ...patch };
};

const removeDocumentTemplateUploadItem = (id: string) => {
  documentTemplateUploadItems.value = documentTemplateUploadItems.value.filter(item => item.id !== id);
};

const removeDocumentTemplateUploadItemLater = (id: string) => {
  window.setTimeout(() => removeDocumentTemplateUploadItem(id), 1400);
};

const clearDocumentTemplateUploadItems = (
  format: DocumentTemplateFormatKey,
  role: DocumentTemplateFileRole,
) => {
  documentTemplateUploadItems.value = documentTemplateUploadItems.value.filter(
    item => !(item.format === format && item.role === role && item.status !== 'reading'),
  );
};

const normalizeDocumentTemplateConfig = (value?: DocumentTemplateConfig | null): DocumentTemplateConfig => {
  const src = value || {};
  const normalizeFormat = (key: DocumentTemplateFormatKey, format?: DocumentTemplateFormatConfig): DocumentTemplateFormatConfig => ({
    requirement_file: format?.requirement_file ?? null,
    template_files: Array.isArray(format?.template_files) ? format!.template_files!.slice(0, documentTemplateReferenceLimit(key)) : [],
  });
  return {
    word: normalizeFormat('word', src.word),
    excel: normalizeFormat('excel', src.excel),
    pdf: normalizeFormat('pdf', src.pdf),
    ppt: normalizeFormat('ppt', src.ppt),
  };
};

const ensureDocumentTemplateConfig = () => {
  if (!formData.value.config.document_template) {
    formData.value.config.document_template = createDefaultDocumentTemplateConfig();
    return;
  }
  const cfg = formData.value.config.document_template as DocumentTemplateConfig;
  (['word', 'excel', 'pdf', 'ppt'] as DocumentTemplateFormatKey[]).forEach((key) => {
    if (!cfg[key]) {
      cfg[key] = { requirement_file: null, template_files: [] };
    }
    if (cfg[key]!.requirement_file === undefined) {
      cfg[key]!.requirement_file = null;
    }
    if (!Array.isArray(cfg[key]!.template_files)) {
      cfg[key]!.template_files = [];
    }
    const limit = documentTemplateReferenceLimit(key);
    if (cfg[key]!.template_files!.length > limit) {
      cfg[key]!.template_files = cfg[key]!.template_files!.slice(0, limit);
    }
  });
};

const getDocumentTemplateFormat = (format: DocumentTemplateFormatKey): DocumentTemplateFormatConfig => {
  ensureDocumentTemplateConfig();
  const cfg = formData.value.config.document_template as DocumentTemplateConfig;
  if (!cfg[format]) {
    cfg[format] = { requirement_file: null, template_files: [] };
  }
  if (!Array.isArray(cfg[format]?.template_files)) {
    cfg[format]!.template_files = [];
  }
  return cfg[format]!;
};

const documentTemplateFileMeta = (file?: DocumentTemplateFile | null): string => {
  if (!file) return '';
  const source = file.source === 'builtin' ? '默认' : '自定义';
  const ext = (file.file_type || '').replace(/^\./, '').toUpperCase() || 'FILE';
  const size = file.file_size ? formatBytes(file.file_size) : '';
  return [source, ext, size].filter(Boolean).join(' · ');
};

const documentRequirementAccept = '.md,.markdown,.txt';
const documentReferenceAccept = (format: DocumentTemplateFormatKey) => {
  if (format === 'word') return '.doc,.docx';
  if (format === 'excel') return '.xls,.xlsx';
  if (format === 'ppt') return '.ppt,.pptx';
  return '.pdf';
};
...
const allowedDocumentRequirementExts = new Set(['md', 'markdown', 'txt']);
const allowedDocumentReferenceExts: Record<DocumentTemplateFormatKey, Set<string>> = {
...
const fileToDocumentTemplateFile = (file: File): Promise<DocumentTemplateFile> => new Promise((resolve, reject) => {
...
const selectLocalFiles = (accept: string, multiple: boolean): Promise<File[]> => new Promise((resolve) => {
...
const validateTemplateFileSize = (file: File, format: DocumentTemplateFormatKey, role: DocumentTemplateFileRole): boolean => {
  const limit = documentTemplateFileSizeLimit(format, role);
  if (file.size <= limit) return true;
  MessagePlugin.error(`模板文件不能超过 ${formatBytes(limit)}：${file.name}`);
  return false;
};

const chooseRequirementFile = async (format: DocumentTemplateFormatKey) => {
  const files = await selectLocalFiles(documentRequirementAccept, false);
  const file = files[0];
  if (!file) return;
  const ext = getFileExt(file.name);
  if (!allowedDocumentRequirementExts.has(ext)) {
    MessagePlugin.error('模板要求文件仅支持 Markdown 或 TXT');
    return;
  }
  if (!validateTemplateFileSize(file, format, 'requirement')) return;
  clearDocumentTemplateUploadItems(format, 'requirement');
  const uploadId = addDocumentTemplateUploadItem(format, 'requirement', file);
  try {
    const templateFile = await fileToDocumentTemplateFile(file);
    getDocumentTemplateFormat(format).requirement_file = templateFile;
    updateDocumentTemplateUploadItem(uploadId, { status: 'done', message: '已读取' });
    removeDocumentTemplateUploadItemLater(uploadId);
  } catch (error) {
    updateDocumentTemplateUploadItem(uploadId, {
      status: 'error',
      message: error instanceof Error ? error.message : '读取失败',
    });
    MessagePlugin.error(`读取文件失败：${file.name}`);
  }
};

const restoreDefaultRequirementFile = (format: DocumentTemplateFormatKey) => {
  getDocumentTemplateFormat(format).requirement_file = { ...documentTemplateDefaults[format] };
};

const removeRequirementFile = (format: DocumentTemplateFormatKey) => {
  getDocumentTemplateFormat(format).requirement_file = null;
};

const chooseReferenceFiles = async (format: DocumentTemplateFormatKey) => {
  const cfg = getDocumentTemplateFormat(format);
  const current = cfg.template_files || [];
  const limit = documentTemplateReferenceLimit(format);
  const remaining = limit - current.length;
  if (remaining <= 0) {
    MessagePlugin.warning(`参考文档最多只能上传 ${limit} 份`);
    return;
  }
  const files = await selectLocalFiles(documentReferenceAccept(format), true);
  if (files.length === 0) return;
  const allowed = allowedDocumentReferenceExts[format];
  const accepted: File[] = [];
  for (const file of files) {
    const ext = getFileExt(file.name);
    if (!allowed.has(ext)) {
      MessagePlugin.error(`${format.toUpperCase()} 参考文档不支持该文件类型：${file.name}`);
      return;
    }
    if (!validateTemplateFileSize(file, format, 'reference')) return;
    accepted.push(file);
  }
  if (accepted.length > remaining) {
    MessagePlugin.warning(`参考文档最多 ${limit} 份，本次只添加前 ${remaining} 份`);
  }
  clearDocumentTemplateUploadItems(format, 'reference');
  for (const file of accepted.slice(0, remaining)) {
    const uploadId = addDocumentTemplateUploadItem(format, 'reference', file);
    try {
      const addition = await fileToDocumentTemplateFile(file);
      const latestCfg = getDocumentTemplateFormat(format);
      latestCfg.template_files = [...(latestCfg.template_files || []), addition];
      updateDocumentTemplateUploadItem(uploadId, { status: 'done', message: '已读取' });
      removeDocumentTemplateUploadItemLater(uploadId);
    } catch (error) {
      updateDocumentTemplateUploadItem(uploadId, {
...
const removeReferenceFile = (format: DocumentTemplateFormatKey, index: number) => {
  const cfg = getDocumentTemplateFormat(format);
  cfg.template_files = (cfg.template_files || []).filter((_, i) => i !== index);
};

const validateDocumentTemplateBeforeSave = (): boolean => {
  if (!isDocumentProcessingAgent.value) {
    delete formData.value.config.document_template;
    return true;
  }
  ensureDocumentTemplateConfig();
  const config = formData.value.config.document_template as DocumentTemplateConfig;
  for (const item of documentTemplateFormats.value) {
    const cfg = config[item.key] || {};
    const req = cfg.requirement_file;
    if (req) {
      const ext = (req.file_type || getFileExt(req.file_name || '')).toLowerCase();
      if (req.source !== 'builtin' && !allowedDocumentRequirementExts.has(ext)) {
        MessagePlugin.error(`${item.label} 模板要求文件仅支持 Markdown 或 TXT`);
        currentSection.value = 'document-template';
        return false;
      }
    }
    const refs = cfg.template_files || [];
    const referenceLimit = documentTemplateReferenceLimit(item.key);
    if (refs.length > referenceLimit) {
      MessagePlugin.error(`${item.label} 参考文档最多只能上传 ${referenceLimit} 份`);
      currentSection.value = 'document-template';
      return false;
    }
    for (const file of refs) {
      const ext = (file.file_type || getFileExt(file.file_name || '')).toLowerCase();
      if (!allowedDocumentReferenceExts[item.key].has(ext)) {
        MessagePlugin.error(`${item.label} 参考文档文件类型不支持：${file.file_name}`);
        currentSection.value = 'document-template';
        return false;
      }
    }
...
const presetKbMismatchReason = (preset: AgentTypePreset): string => {
  if (false) return '知识库管理智能体不支持 FAQ 知识库';
...
//     某类业务预设的 `none_of: [...]`），作为增量合并，而不是整体覆盖；
//   - `all_of` / `none_of` 直接从 YAML 继承（工具不表达这类约束）。
//
// 这样 knowledge-qa / wiki-qa / hybrid 在 YAML 里彻底不写 `kb_filter`，
// 业务预设只需声明额外的 `none_of`，"工具→能力"的映射只在
...
// "知识问答 / RAG 模式"对 KB 的隐式要求：必须有 vector 或 keyword 索引。
...
const filteredKbOptionsForPreset = computed(() => {
  ...
  return kbOptions.value.map(kb => {
    ...
    const ok = presetResult.ok && managerPermissionOK;
...
const applyAgentTypePreset = (preset: AgentTypePreset | null) => {
  ...
  // supported_file_types 采用"强同步"语义：预设显式声明时写入，
  // 其余类型切过来时清空，否则会从上一个类型带过来残留。
...
const normalizeAgentFormData = (agent: CustomAgent) => {
  ...
  if (!agentData.config.agent_mode) {
    ...
    agentData.config.agent_mode = 'agent';
  }
...
watch(() => props.visible, async (val) => {
  if (val) {
    ...
    if (props.mode === 'edit' && props.agent) {
      ...
      // 应用系统默认提示词（根据模式填充）
      const isAgent = newFormData.config.agent_mode === 'agent';
      if (isAgent) {
      ...
      if (newFormData.config.agent_mode === 'agent') {
        ...
        if (defaultTypeId !== 'document-processing-agent') {
          delete formData.value.config.document_template;
        } else {
          ensureDocumentTemplateConfig();
        }
...
const initSkillsSelectionMode = () => {
  if (formData.value.config.lightweight_skills_selection_mode) {
    skillsSelectionMode.value = formData.value.config.lightweight_skills_selection_mode;
  } else if (formData.value.config.skills_selection_mode) {
    skillsSelectionMode.value = formData.value.config.skills_selection_mode;
  } else if (formData.value.config.selected_lightweight_skills?.length > 0 || formData.value.config.selected_skills?.length > 0) {
    skillsSelectionMode.value = 'selected';
  } else {
    skillsSelectionMode.value = 'none';
  }
  if (formData.value.config.professional_skills_selection_mode) {
    professionalSkillsSelectionMode.value = formData.value.config.professional_skills_selection_mode;
  } else if (formData.value.config.selected_professional_skills?.length > 0) {
    professionalSkillsSelectionMode.value = 'selected';
  } else {
...
const fillBuiltinAgentDefaults = () => {
  ...
  const isAgent = config.agent_mode === 'agent';
...
watch(skillsSelectionMode, (mode) => {
  formData.value.config.lightweight_skills_selection_mode = mode;
  // Legacy fields are kept only for reading old agents. New saves use the
  // lightweight fields so prompt skills are not exposed as read_skill tools.
  formData.value.config.skills_selection_mode = 'none';
  formData.value.config.selected_skills = [];
  if (mode === 'none') {
    formData.value.config.selected_lightweight_skills = [];
  } else if (mode === 'all') {
    formData.value.config.selected_lightweight_skills = [];
  }
  // selected 模式保持 selected_lightweight_skills 不变
});

watch(professionalSkillsSelectionMode, (mode) => {
...
const loadDependencies = async () => {
  try {
    await Promise.all([
      ...
      listDatabaseSources().then(res => {
        dbSourceOptions.value = res.data || [];
      }).catch(() => {
        dbSourceOptions.value = [];
      }),
    ]);
    ...
    professionalSkillsAvailable.value = editorResources.professionalSkillsAvailable;
    professionalSkillOptions.value = editorResources.professionalSkills;
    ...
    const placeholders = editorResources.placeholders as Partial<PlaceholdersResponse> | null;
    placeholderData.value = {
      system_prompt: placeholders?.system_prompt ?? [],
      agent_system_prompt: placeholders?.agent_system_prompt ?? [],
    };
...
const openDataSourcePage = () => {
  ...
  router.push('/platform/data-sources');
};
...
// 处理点击占位符标签
const handlePlaceholderClick = (_type: 'system', placeholderName: string) => {
...
const handleSave = async () => {
  ...
  if (!validateDocumentTemplateBeforeSave()) {
    return;
  }

  {
    ...
    const llmCallTimeout = Number(formData.value.config.llm_call_timeout || 0);
    if (!Number.isFinite(llmCallTimeout) || llmCallTimeout < 0 || llmCallTimeout > AGENT_LLM_TIMEOUT_LIMIT_SECONDS) {
      MessagePlugin.error(`LLM 调用超时必须在 0-${AGENT_LLM_TIMEOUT_LIMIT_SECONDS} 秒之间`);
      currentSection.value = 'model';
      return;
    }
  }

  if (isDataAnalysisAgent.value && (!formData.value.config.db_data_sources || formData.value.config.db_data_sources.length === 0)) {
    MessagePlugin.error('请至少绑定一个数据源');
...
.document-template-panel__header {
  display: flex;
  justify-content: space-between;
  align-items: flex-start;
  gap: 12px;
...
.document-template-panel__title {
  font-size: 14px;
  font-weight: 600;
  color: var(--td-text-color-primary);
  line-height: 1.4;
...
.document-template-panel__actions {
  display: flex;
  align-items: center;
  gap: 6px;
  flex-shrink: 0;
...
.document-template-file-list {
  display: flex;
  flex-direction: column;
  gap: 8px;
}

.document-template-upload-list {
  display: flex;
  flex-direction: column;
  gap: 8px;
  margin-bottom: 8px;
...
.document-template-upload-item--done {
  border-color: var(--td-success-color-4);
  background: var(--td-success-color-1);
}

.document-template-upload-item--error {
  border-color: var(--td-error-color-4);
  background: var(--td-error-color-1);
}

.document-template-upload-item__main {
  display: flex;
  align-items: center;
  gap: 8px;
  min-width: 0;
...
.document-template-file__text {
  min-width: 0;
}

.document-template-file__name {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
  font-size: 13px;
