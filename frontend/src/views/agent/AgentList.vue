        ...
        <div class="shared-detail-drawer">
          ...
          <div class="shared-detail-drawer-body">
            ...
            <template v-if="currentSharedAgent.agent?.config">
              ...
              <div class="shared-detail-row">
                <span class="shared-detail-label">{{ $t('agent.shareScope.dataSource') }}</span>
                <span class="shared-detail-value">{{ sharedAgentDataSourceScopeText }}</span>
              </div>
              <div class="shared-detail-row">
...
const sharedAgentDataSourceScopeText = computed(() => {
  const sources = currentSharedAgent.value?.agent?.config?.db_data_sources
  if (Array.isArray(sources) && sources.length > 0) {
    return t('agent.shareScope.dataSourceSelected', { count: sources.length })
  }
  return t('agent.shareScope.dataSourceNone')
})
