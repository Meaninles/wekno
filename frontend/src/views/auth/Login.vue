          ...
          <div class="form-content">
            <t-form ref="formRef" :data="formData" :rules="formRules" @submit="handleLogin" layout="vertical">
              <t-form-item :label="$t('auth.username')" name="username">
                <t-input v-model="formData.username" :placeholder="$t('auth.usernamePlaceholder')" type="text"
              ...
              <div v-if="oidcEnabled || customSSOEnabled" class="oidc-divider">
              ...
              <t-button v-if="customSSOEnabled" theme="primary" size="large" block :loading="customSSOLoading"
                :disabled="loading" class="oidc-button custom-sso-button" @click="handleCustomSSOLogin">
                {{ customSSOLoading ? $t('auth.redirectingToOIDC') : customSSOLoginText }}
              </t-button>
            </t-form>
...
const customSSOLoading = ref(false)
const isRegisterMode = ref(false)
const oidcEnabled = ref(false)
const oidcProviderName = ref('')
const customSSOEnabled = ref(false)
const customSSOProviderName = ref('')
...
const getCustomSSORedirectURI = () => `${window.location.origin}/api/v1/custom/iam/sso/callback`
...
const loadCustomSSOConfig = async () => {
  try {
    const response = await getCustomSSOConfig()
    customSSOEnabled.value = !!response.success && !!response.enabled
    customSSOProviderName.value = response.provider_display_name || ''
  } catch {
    customSSOEnabled.value = false
    customSSOProviderName.value = ''
  }
}
...
const handleCustomSSOLogin = async () => {
  try {
    customSSOLoading.value = true
    const response = await getCustomSSOAuthorizationURL(getCustomSSORedirectURI())
    const authorizationURL = response.authorization_url
