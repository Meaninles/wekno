<template>
  <div ref="rootElement" class="agent-stream-display" :class="{ 'is-embedded': embeddedMode, 'is-rag-mode': ragMode }">
    ...
    <div v-if="!ragMode || displayEvents.length > 0 || showAgentActivityIndicator" ref="streamingStepsContainer"
            ...
            <div v-else-if="event.type === 'answer' && (event.done || (event.content && event.content.trim()))"
              ...
              <div v-if="event.content && event.content.trim()" class="answer-content markdown-content">
                <template
                  ...
                  :key="`answer-segment-${event.event_id || index}-${segmentIndex}`">
                  <div v-if="segment.kind === 'markdown' && segment.html.trim()" v-stable-html="segment.html"></div>
                  <div v-else-if="segment.kind === 'structured_result'" class="inline-structured-result">
                    <ToolResultRenderer :display-type="segment.result.display_type" :tool-data="segment.result.tool_data"
                      :output="segment.result.output" :arguments="segment.result.arguments" :embedded-mode="embeddedMode"
                      :embed-channel-id="embedChannelId" :embed-token="embedToken" :embed-session-id="sessionId"
                      :embed-session-sig="embedSessionSig" />
                  </div>
                </template>
              ...
              <div v-if="event.done && event.content && event.content.trim() && !embeddedMode && !shareMode"
                ...
                <AnswerFeedbackButtons v-if="sessionId && session?.id" :session-id="sessionId" :message-id="session.id"
                  :initial-feedback="session.answer_feedback" :target="session" />
                <t-tooltip v-if="event.is_fallback" :content="$t('chat.fallbackHint')" placement="top">
                  <t-button size="small" variant="outline" shape="round" class="fallback-icon-btn">
                    <t-icon name="info-circle" />
                  </t-button>
                </t-tooltip>
                <ChatRequestInfoButton v-if="showRequestInfo && isConversationDone" :session="requestInfoSession"
      ...
      <div v-for="(result, resultIndex) in promotedStructuredResults" :key="`promoted-result-${resultIndex}`"
        class="promoted-tool-result">
        <ToolResultRenderer :display-type="result.display_type" :tool-data="result.tool_data" :output="result.output"
...
const TOOL_NAME_KEYS: Record<string, string> = {
  ...
  table_analysis: 'agentStream.tools.tableAnalysis',
  table_schema: 'agentStream.tools.tableSchema',
  table_analysis_display_intent: 'agentStream.tools.tableAnalysisDisplayIntent',
  database_query: 'agentStream.tools.databaseQuery',
  db_catalog: 'agentStream.tools.dbCatalog',
  db_schema: 'agentStream.tools.dbSchema',
  db_query: 'agentStream.tools.dbQuery',
};
...
type PromotedResultBlock = {
  display_type: 'structured_analysis_result' | 'general_agent_artifacts';
  tool_data: Record<string, any>;
  output?: string;
  arguments?: Record<string, any>;
};

type StructuredResultBlock = PromotedResultBlock & {
  display_type: 'structured_analysis_result';
};

type AnswerRenderSegment =
  | { kind: 'markdown'; html: string }
  | { kind: 'structured_result'; result: StructuredResultBlock };
...
const isConversationDone = computed(() => {
  ...
  const errorEvent = stream.find((e: any) => e.type === 'error' && e.done !== false);
  if (errorEvent) {
    return true;
  }
...
const activeAnswerMarkdown = computed(() => {
  ...
  const answers = source.filter((e: any) => e.type === 'answer' && !e.superseded);
...
const isTransientStatusEvent = (event: any): boolean => {
  if (!event || event.type !== 'tool_call') return false;
  return Boolean(
    event.transient_status === true ||
    event.agent_progress?.transient === true ||
    event.tool_data?.transient === true ||
    event.tool_name === 'assistant_status',
  );
};
...
function getThinkingSummary(event: any): string {
...
const structuredChartInfos = computed<StructuredChartInfo[]>(() => (
  structuredAnalysisResults.value.map((result) => {
    const chart = result.tool_data?.chart || {};
...
const completedAnswerMarkdown = computed(() => {
  ...
  const result = buildFullEventList(stream);
  return result
    .filter((event: any) => event.type === 'answer' && !event.superseded && event.content)
    .map((event: any) => String(event.content || ''))
    .join('\n\n');
});

const inlineStructuredResultIndexes = computed(() => {
  if (!completedAnswerMarkdown.value) return new Set<number>();
  const split = splitStructuredChartMarkdown(
    unwrapFinalAnswerWrappers(completedAnswerMarkdown.value),
    structuredAnalysisResults.value.length,
    structuredChartInfos.value,
  );
  return new Set(split.usedResultIndexes);
});

const hasExplicitStructuredChartPlaceholders = computed(() => (
  /\{\{\s*chart\s*:\s*([a-zA-Z0-9_-]+|\d+)\s*\}\}/.test(
    unwrapFinalAnswerWrappers(completedAnswerMarkdown.value),
  )
));

const promotedStructuredResults = computed<PromotedResultBlock[]>(() => [
  ...(hasExplicitStructuredChartPlaceholders.value
    ? []
    : structuredAnalysisResults.value.filter((_, index) => !inlineStructuredResultIndexes.value.has(index))),
  ...generalAgentArtifactResults.value,
]);
...
const displayEvents = computed(() => {
  ...
  if (terminalError) {
    return answerEvents.length > 0 ? [...answerEvents, terminalError] : [terminalError];
  }
...
const getToolError = (event: any): string => {
...
const hasResults = (event: any): boolean => {
  if (getToolError(event)) return true;
...
// 自定义渲染器 - 支持 Mermaid，并过滤模型生成的无效 Markdown 图片占位。
const agentRenderer = createChatMarkdownRenderer({
  codeRenderer: createMermaidCodeRenderer('mermaid-agent'),
...
const renderAnswerSegments = (content: unknown): AnswerRenderSegment[] => {
  const contentStr = typeof content === 'string' ? content : String(content || '');
  const unwrapped = unwrapFinalAnswerWrappers(contentStr);
  if (!unwrapped.trim()) return [];

  if (!isConversationDone.value) {
    return [{ kind: 'markdown', html: renderMarkdownContent(unwrapped) }];
  }
  ...
  return split.segments
    .map((segment): AnswerRenderSegment | null => {
      if (segment.kind === 'chart') {
        const result = structuredAnalysisResults.value[segment.resultIndex];
        return result ? { kind: 'structured_result', result } : null;
      }
      if (!segment.content) return null;
      return { kind: 'markdown', html: renderMarkdownContent(segment.content) };
    })
    .filter((segment): segment is AnswerRenderSegment => Boolean(segment));
};
...
const getAgentProgressMessage = (event: any): string => {
  return String(
    event?.agent_progress?.message ||
...
const getToolTitle = (event: any): string => {
  const agentProgressMessage = getAgentProgressMessage(event);
...
.action-name.agent-progress-title,
.agent-progress-history-message {
  display: -webkit-box;
  -webkit-box-orient: vertical;
  -webkit-line-clamp: 2;
  line-clamp: 2;
...
.tool-output-wrapper {
  ...
  &.tool-error-output {
    .detail-output-wrapper {
      border-color: var(--td-error-color-6, var(--td-error-color));
    }
...
// Compact timeline instead of boxed cards.
.agent-stream-display {
  .tool-event {
    .action-card {
      background: transparent;
