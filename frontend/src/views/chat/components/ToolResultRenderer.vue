<template>
  <div class="tool-result-renderer">
    ...
    <DatabaseMetadataResult v-else-if="displayType === 'db_catalog' || displayType === 'db_schema'"
      :data="toolData as DatabaseMetadataData" />

    <!-- Structured CSV/Database Analysis Display -->
    <StructuredAnalysisResult v-else-if="displayType === 'structured_analysis_result'"
      :data="toolData as StructuredAnalysisData" />

    <!-- General Agent Artifacts Display -->
    <GeneralAgentArtifactsResult v-else-if="displayType === 'general_agent_artifacts'"
      :data="toolData as GeneralAgentArtifactsData" :embedded-mode="embeddedMode"
      :embed-channel-id="embedChannelId" :embed-token="embedToken" :embed-session-id="embedSessionId"
...
import DatabaseMetadataResult from './tool-results/DatabaseMetadataResult.vue';
import StructuredAnalysisResult from './tool-results/StructuredAnalysisResult.vue';
import GeneralAgentArtifactsResult from './tool-results/GeneralAgentArtifactsResult.vue';
...
const embeddedMode = computed(() => props.embeddedMode === true);
const embedChannelId = computed(() => props.embedChannelId || '');
const embedToken = computed(() => props.embedToken || '');
const embedSessionId = computed(() => props.embedSessionId || '');
const embedSessionSig = computed(() => props.embedSessionSig || '');
</script>
