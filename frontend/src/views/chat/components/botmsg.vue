    ...
    <div class="bot_msg" :class="{ 'is-embedded': embeddedMode }">
        ...
        <div ref="parentMd" v-if="!session.hideContent && !session.isAgentMode">
            ...
            <div v-if="!shareMode && session.is_completed && (content || session.content)" class="answer-toolbar">
                ...
                <AnswerFeedbackButtons v-if="!embeddedMode && sessionId && session?.id" :session-id="sessionId"
                    :message-id="session.id" :initial-feedback="session.answer_feedback" :target="session" />
                <!-- Fallback 提示图标 -->
