...
const sendMsg = async (value, modelId = '', mentionedItems = [], imageFiles = [], attachmentFiles = []) => {
    ...
    const selectedTags = props.embeddedMode ? [] : (useSettingsStoreInstance.settings.selectedTags || []);
    const kbIdSet = new Set(sidebarKbIds);
    const fileIdSet = new Set(sidebarFileIds);
    const tagIdSet = new Set();
    for (const tag of selectedTags) {
        if (tag?.id) tagIdSet.add(tag.id);
        if (tag?.kbId) kbIdSet.add(tag.kbId);
    }
    for (const item of mentionedItems || []) {
        ...
        if (item.type === 'kb' && !kbIdSet.has(item.id)) {
            ...
            const tagKbId = item.kbId || item.kb_id;
            if (tagKbId) kbIdSet.add(tagKbId);
        }
    }
    const kbIds = [...kbIdSet];
