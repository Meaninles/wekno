...
import { getSessionsList, createSessions } from "@/api/chat/index";
...
const editingTagInputRefs = new Map<string, any>();
...
const tagMap = computed<Record<string, KnowledgeTag>>(() => {
  const map: Record<string, KnowledgeTag> = {};
...
const selectedTagId = computed(() => selectedTagIds.value[0] || '');
const filteredTags = computed(() => sidebarTags.value);

const focusTdesignInput = (target: any) => {
  const input = target?.inputElement || target?.$el?.querySelector?.('input') || target?.querySelector?.('input');
...
const startCreateTag = async () => {
  ...
  newTagName.value = '';
  await nextTick();
  focusTdesignInput(newTagInputRef.value);
};
...
const setEditingTagInputRefByTag = (tagId: string) => (el: any) => {
  if (el) {
...
const startEditTag = async (tag: KnowledgeTag) => {
  creatingTag.value = false;
  creatingTagLoading.value = false;
  editingTagId.value = tag.id;
  editingTagName.value = tag.name;
  await nextTick();
  focusTdesignInput(editingTagInputRefs.get(tag.id));
};
...
const submitEditTag = async () => {
  if (!kbId.value || !editingTagId.value) return;
...
const confirmDeleteTag = (tag: KnowledgeTag) => {
  if (!kbId.value) return;
  const tagSeqId = Number(tag.seq_id ?? tag.id);
  if (!Number.isFinite(tagSeqId)) {
    MessagePlugin.error(t('common.operationFailed'));
    return;
  }
  let dialog: ReturnType<typeof DialogPlugin.confirm> | undefined;
  dialog = DialogPlugin.confirm({
    header: t('knowledgeBase.tagDeleteTitle') as string,
    body: t(isFAQ.value ? 'knowledgeBase.tagDeleteDesc' : 'knowledgeBase.tagDeleteDescDoc', { name: tag.name }) as string,
    confirmBtn: { content: t('common.confirm') as string, theme: 'danger' },
    cancelBtn: { content: t('common.cancel') as string },
    onConfirm: async () => {
      try {
        await deleteKnowledgeBaseTag(kbId.value, tagSeqId, { force: true });
        if (selectedTagIds.value.includes(tag.id)) {
          await handleTagFilterChange(selectedTagIds.value.filter((id) => id !== tag.id));
        }
