  ...
  <Teleport to="body">
                      ...
                      <div class="members-list">
                        <div v-for="member in filteredMembers" :key="member.id" class="member-item" :class="{
                          ...
                          <div class="member-info">
                            ...
                            <span class="member-secondary">{{ memberSecondaryLabel(member) }}</span>
                          </div>
    ...
    <t-dialog v-model:visible="showAddMemberDialog" :header="$t('organization.addMember.dialogTitle')"
      ...
      <div class="add-member-dialog">
        <t-radio-group v-model="addMemberMode" variant="default-filled" class="add-member-mode">
          <t-radio-button v-for="option in addMemberModeOptions" :key="option.value" :value="option.value">
            {{ option.label }}
          </t-radio-button>
        </t-radio-group>

        <p class="add-member-tip">{{ addMemberTip }}</p>

        <div v-if="addMemberMode === 'tenant'" class="add-member-field">
        ...
        <SpaceMemberBatchPicker v-else :key="`${addMemberMode}-${batchPickerKey}`" :space-id="props.orgId || ''"
          :mode="addMemberMode === 'organization' ? 'organization' : 'member'"
          @update:candidates="selectedBatchCandidates = $event"
          @update:duplicate-count="addMemberDuplicateTenantCount = $event" />
        ...
        <p v-if="addMemberDuplicateTenantCount > 0" class="field-hint add-member-summary">
          {{ $t('organization.addMember.duplicateTenantHint', { count: addMemberDuplicateTenantCount }) }}
        </p>
      </div>
    </t-dialog>
  </Teleport>
...
} from '@/custom/modules/iam/api'
import SpaceMemberBatchPicker from '@/custom/modules/iam/SpaceMemberBatchPicker.vue'
...
const selectedBatchCandidates = ref<IAMSpaceMemberCandidate[]>([])
const addMemberDuplicateTenantCount = ref(0)
const batchPickerKey = ref(0)
...
const addMemberTip = computed(() => {
  if (addMemberMode.value === 'member') return t('organization.addMember.tipMember')
  if (addMemberMode.value === 'organization') return t('organization.addMember.tipOrganization')
  return t('organization.addMember.tipTenant')
})
...
const selectedInviteCandidates = computed(() => selectedBatchCandidates.value)

const addMemberConfirmDisabled = computed(() => {
  if (addMemberMode.value === 'tenant') return selectedTenantId.value == null
  return selectedInviteCandidates.value.length === 0
})

const addMemberConfirmText = computed(() => {
  if (addMemberMode.value === 'tenant') return t('organization.addMember.confirmBtn')
  return t('organization.addMember.confirmBatchBtn', { count: selectedInviteCandidates.value.length })
})
...
const handleAddMember = async () => {
  if (!props.orgId || addMemberConfirmDisabled.value) return

  addMemberSubmitting.value = true
  try {
    if (addMemberMode.value === 'tenant') {
    ...
    if (successCount > 0) {
      MessagePlugin.success(t('organization.addMember.batchSuccess', { count: successCount }))
      showAddMemberDialog.value = false
      resetAddMemberDialog()
      fetchMembers()
      fetchOrgDetail()
    ...
    if (failedCount > 0) {
      MessagePlugin.error(t('organization.addMember.batchFailed', { count: failedCount }))
    }
  } catch (error: any) {
