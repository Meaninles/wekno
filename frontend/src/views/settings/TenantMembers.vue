                ...
                <t-popconfirm v-if="row.status === 'pending'" theme="warning"
                  :content="row.is_share_link
                    ...
                    : $t('tenantInvitation.revoke.confirmBody', {
                        username: row.invitee_name || row.invitee_user_id,
                      })"
      ...
      <div class="members-list-wrap">
        <div class="members-list-header">
          ...
          <div class="members-list-actions">
              ...
              <template #content>
                <div class="member-invite-popup-inner" @click.stop>
                  ...
                  <t-form v-if="addDialogStep === 'form'" ref="addFormRef" :data="addForm" :rules="addFormRules"
                    ...
                    <t-form-item :label="$t('tenantMember.add.usernameLabel')" name="username">
                      <t-input v-model="addForm.username" :placeholder="$t('tenantMember.add.usernamePlaceholder')"
                        clearable />
                    </t-form-item>
            ...
            <!-- Share-link generator. Sits next to the invite-by-username
                 popup so the two flows live side-by-side: "I know who"
                 (username input) vs "I don't" (one link, group chat). -->
            ...
            <t-table row-key="user_id" :data="members" :columns="columns" size="medium" hover stripe :loading="loading">
              <template #member="{ row }">
                <div class="member-cell">
                  ...
                  <span v-if="memberSecondary(row)" class="member-secondary">{{ memberSecondary(row) }}</span>
                </div>
              </template>
              <template #role="{ row }">
                <div class="role-cell">
              ...
              <template #actions="{ row }">
                <t-popconfirm
                  ...
                  :content="$t('tenantMember.remove.confirmBody', { name: row.username })"
...
// share-link generator state (separate popup next to the username
...
// Two-step invite inside the popup: 'form' renders the username/role inputs;
...
const memberDisplayByUserId = reactive<Record<string, { username?: string }>>({})
...
const addForm = reactive<{ username: string; role: TenantRole }>({
  username: '',
  role: 'contributor',
})
...
function memberPrimary(row: { username?: string }) {
  return row.username?.trim() || '—'
}

function memberSecondary(row: { username?: string }) {
  return ''
}

const addFormRules = {
  username: [
    { required: true, message: t('tenantMember.errors.usernameRequired'), trigger: 'blur' },
  ],
...
function rememberMembersForAudit(rows: TenantMember[]) {
  for (const m of rows) {
    memberDisplayByUserId[m.user_id] = { username: m.username }
  }
}
...
function inviteePrimary(row: TenantInvitation): string {
  return row.invitee_name?.trim() || row.invitee_user_id
}

function inviterPrimary(row: TenantInvitation): string {
  return row.inviter_name?.trim() || row.invited_by || '—'
}
...
const addConfirmUsername = computed(() => addForm.username.trim())
...
async function submitAdd() {
  ...
  await sendInvitation(addForm.username.trim(), addForm.role)
}
...
async function sendInvitation(username: string, role: TenantRole) {
  adding.value = true
  try {
    const resp = await createInvitation(activeTenantId.value, { username, role })
    if (resp.success) {
      invitationsPage.value = 1
      await loadInvitations()
 ...
 * a malformed per-user invitation with a missing username. */
.share-link-title {
  display: inline-flex;
  align-items: center;
  gap: 4px;
