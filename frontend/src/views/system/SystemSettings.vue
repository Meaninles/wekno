  ...
  <div class="system-settings">
    ...
    <div v-else class="settings-group">
      <!--
        ...
        (a tag-input with one entry per username). NOT a system_setting
...
//   - adminUsernames: the v-model bound to the t-tag-input (excludes
//     current user; that's the visible source of truth).
//   - adminUsernameToId: username → user UUID, populated from the list
//     endpoint. Needed because revoke takes a UUID, not a username.
...
const adminUsernames = ref<string[]>([])
const adminUsernameToId = ref<Record<string, string>>({})
...
function modifiedMeta(item: SystemSettingItem): string {
  ...
  // Prefer the resolved username the server enriches via
...
// loadAdmins refreshes the admin tag list + the username→id lookup
...
async function loadAdmins() {
  try {
    ...
    for (const u of resp.admins ?? []) {
      // Empty usernames would collapse to a single tag "" that can't be
    ...
    adminUsernameToId.value = map
    adminUsernames.value = usernames
  } catch (err: any) {
...
function confirmAdminChange(action: 'promote' | 'revoke', username: string): Promise<boolean> {
  const base = `system.globalSettings.admins.confirm.${action}`
  return adminPopconfirm.ask({
    content: globalSettingsText(`${base}.body`, { username }),
...
async function onAdminsChange(next: string[]) {
  ...
  // Snapshot of what's currently authoritative — the username→id map's
  ...
  for (const username of Object.keys(adminUsernameToId.value)) {
    if (adminUsernameToId.value[username] !== currentUserId.value) {
      authoritative.add(username)
    }
  }
  const nextSet = new Set(next.map((e) => e.trim()).filter(Boolean))

  // Drop the user-typed entry to canonical trim before we
  // diff. We preserve server-returned username case because the
  ...
  const added: string[] = []
  for (const username of nextSet) {
    if (!authoritative.has(username)) added.push(username)
  }
  const removed: string[] = []
  for (const username of authoritative) {
    if (!nextSet.has(username)) removed.push(username)
  }
  ...
  for (const username of added) {
    const ok = await confirmAdminChange('promote', username)
    if (!ok) {
      await loadAdmins()
      return
    }
  ...
  for (const username of removed) {
    const userId = adminUsernameToId.value[username]
    if (!userId) continue
    const ok = await confirmAdminChange('revoke', username)
    if (!ok) {
      await loadAdmins()
      return
    }
  ...
  try {
    for (const username of added) {
      await promoteUserToSystemAdmin({ username })
      applied++
    }
    for (const username of removed) {
      const userId = adminUsernameToId.value[username]
...
function auditActorLabel(userId: string): string {
  ...
  if (me && me.id === userId) {
    return me.username?.trim() || userId.slice(0, 8)
  }
...
//   - admin_promoted/revoked:        username of the affected user
function auditTargetKey(row: AuditLog): string {
  ...
  if (row.action === 'system.admin_promoted' || row.action === 'system.admin_revoked') {
    ...
    return name || (row.target_user_id ? row.target_user_id.slice(0, 8) : '')
  }
