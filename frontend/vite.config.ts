...
function silenceViteClientDebug(): Plugin {
  ...
  const mutedClientDebugLines = [
    `${debugMethod}("[vite] connecting...");`,
    `${debugMethod}(\`[vite] connected.\`);`,
    `${debugMethod}("[vite] connected from window");`,
  ]

  return {
    name: 'silence-vite-client-debug',
    apply: 'serve',
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        const rawUrl = req.url ?? ''
        const path = rawUrl.includes('?') ? rawUrl.slice(0, rawUrl.indexOf('?')) : rawUrl
        if (path !== '/@vite/client') {
          next()
          return
        }
        ...
        ;(res as any).end = (chunk: any, ...args: any[]) => {
          ...
          for (const line of mutedClientDebugLines) {
