...
var toolDisplayNames = map[string]string{
	...
	agenttools.ToolTableAnalysis:       "表格分析",
	agenttools.ToolTableSchema:         "查看表格结构",
...
func (e *AgentEngine) executeToolCalls(
	...
	if e.config.ParallelToolCalls && n >= 2 {
		e.executeToolCallsParallel(ctx, response, step, iteration, sessionID, assistantMessageID, originalUserQuery)
		return
	}

	for i, tc := range response.ToolCalls {
		e.executeSingleToolCall(ctx, tc, i, step, iteration, round, sessionID, assistantMessageID, originalUserQuery)
	}
}
...
func (e *AgentEngine) executeToolCallsParallel(
	...
	for i, tc := range response.ToolCalls {
		...
		g.Go(func() error {
			toolCall := e.runToolCall(gCtx, tc, i, iteration, round, sessionID, assistantMessageID, originalUserQuery)
			mu.Lock()
			results[i] = toolCall
			mu.Unlock()
...
func (e *AgentEngine) executeSingleToolCall(
	...
	toolCall := e.runToolCall(ctx, tc, i, iteration, round, sessionID, assistantMessageID, originalUserQuery)
	step.ToolCalls = append(step.ToolCalls, toolCall)
...
func (e *AgentEngine) runToolCall(
	ctx context.Context, tc types.LLMToolCall, i int,
	iteration, round int, sessionID, assistantMessageID string, originalUserQuery string,
) types.ToolCall {
	...
	execTimeout := toolExecTimeout(tc.Function.Name)
