...
func newCSVTestDuckDB(t *testing.T) *sql.DB {
	t.Helper()
...
func TestLoadFromCSV_GB18030NormalizesForDuckDB(t *testing.T) {
	db := newCSVTestDuckDB(t)

	input := "时间,基准收益,策略收益,当日盈利,超额收益(%),active_count,cash_ratio\n2020-01-02 16:00:00,1.29,0,0,-1.27,0,1\n"
	data, _, err := transform.Bytes(simplifiedchinese.GB18030.NewEncoder(), []byte(input))
	if err != nil {
	...
	tool := &DataAnalysisTool{
		...
		db:        db,
		sessionID: "test-gb18030-csv",
	}

	ctx := context.Background()
	schema, err := tool.LoadFromCSV(ctx, path, "t_gb18030")
	if err != nil {
	...
	for _, want := range []string{"时间", "基准收益", "策略收益", "超额收益(%)", "active_count", "cash_ratio"} {
		if !colSet[want] {
			t.Fatalf("expected column %q, got columns=%v", want, schema.Columns)
		}
	}

	var got string
	if err := db.QueryRowContext(ctx, `SELECT "时间" FROM "t_gb18030" LIMIT 1`).Scan(&got); err != nil {
		t.Fatalf("query Chinese column: %v", err)
	}
	if got != "2020-01-02 16:00:00" {
		t.Fatalf("row value mismatch: got %q", got)
	}
}
