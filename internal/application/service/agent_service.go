...
func (s *agentService) registerMCPTools(
	...
	case "selected":
		if len(config.MCPServices) == 0 {
			logger.Infof(ctx, "MCP selection mode is selected but no services are configured")
			return
		}
	...
	default:
		logger.Warnf(ctx, "Unknown MCP selection mode %q; MCP tools are disabled", mcpMode)
		return
	}
...
func (s *agentService) registerTools(
	...
	if !hasKnowledge {
		...
		// If no knowledge and no web search, also disable todo_write for simple chat.
	...
	for _, toolName := range allowedTools {
		...
		default:
			if !isRuntimeToolRecognized(ctx, config, toolName) {
	...
	registerRuntimeTools(ctx, registry, config, sessionID)
