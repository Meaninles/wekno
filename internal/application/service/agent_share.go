...
var (
	...
	ErrAgentNotConfigured      = errors.New("agent is not fully configured (missing required chat model, required data source, or rerank model when the knowledge_search tool is enabled)")
)
...
func (s *agentShareService) ShareAgent(ctx context.Context, agentID string, orgID string, userID string, tenantID uint64, permission types.OrgMemberRole) (*types.AgentShare, error) {
	...
	if agent.Config.AgentType == types.AgentTypeDataAnalysis && len(agent.Config.DBDataSources) == 0 {
		return nil, ErrAgentNotConfigured
	}
