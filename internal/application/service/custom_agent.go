...
var dbAnalysisToolNames = map[string]bool{
	tools.ToolDBCatalog: true,
	tools.ToolDBSchema:  true,
	tools.ToolDBQuery:   true,
}
...
func normalizeCustomAgentDataSourceConfig(agent *types.CustomAgent) error {
	...
	filtered := make([]string, 0, len(agent.Config.AllowedTools))
	for _, toolName := range agent.Config.AllowedTools {
		if dbAnalysisToolNames[toolName] {
			continue
		}
		filtered = append(filtered, toolName)
	}
	agent.Config.AllowedTools = filtered
	return nil
}
...
func refreshBuiltinAgentMetadata(ctx context.Context, agent *types.CustomAgent, tenantID uint64) *types.CustomAgent {
	...
	refreshed := *agent
	refreshed.Name = defaultAgent.Name
...
func (s *customAgentService) CreateAgent(ctx context.Context, agent *types.CustomAgent) (*types.CustomAgent, error) {
	...
	// agents. Terminal identities cannot be recorded as account owners.
	agent.CreatedBy, _ = types.AccountUserIDFromContext(ctx)
	...
	if err := normalizeCustomAgentDocumentTemplateConfig(agent); err != nil {
		return nil, err
	}
...
func (s *customAgentService) CopyAgent(ctx context.Context, id string) (*types.CustomAgent, error) {
	...
	newAgent.CreatedBy, _ = types.AccountUserIDFromContext(ctx)
