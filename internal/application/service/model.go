...
// ErrModelAlreadyExists is returned when the tenant already has the same model
// configuration.
...
func (s *modelService) CreateModel(ctx context.Context, model *types.Model) error {
	if err := model.ValidateInputCapability(); err != nil {
		return err
	}
	...
	if existing, err := s.findDuplicateModel(ctx, model); err != nil {
		...
		*model = *existing
		return fmt.Errorf("%w: %s", ErrModelAlreadyExists, existing.ID)
	}
...
func (s *modelService) findDuplicateModel(ctx context.Context, model *types.Model) (*types.Model, error) {
	models, err := s.repo.List(ctx, model.TenantID, model.Type, model.Source)
	if err != nil {
		return nil, err
	}

	candidateKey := modelDuplicateKey(model)
	for _, existing := range models {
		if existing == nil || existing.ID == model.ID {
			continue
		}
		if modelDuplicateKey(existing) == candidateKey {
			return existing, nil
		}
	}
	return nil, nil
...
func modelDuplicateKey(model *types.Model) string {
	params := model.Parameters
	embedding := params.EmbeddingParameters
	return strings.Join([]string{
		normalizeModelKeyPart(string(model.Type)),
		normalizeModelKeyPart(string(model.Source)),
		normalizeModelKeyPart(model.Name),
		normalizeModelKeyPart(model.DisplayName),
		normalizeModelKeyPart(params.Provider),
		normalizeModelURLKeyPart(params.BaseURL),
		normalizeModelKeyPart(params.InterfaceType),
		normalizeModelKeyPart(params.ParameterSize),
		strconv.Itoa(embedding.Dimension),
		strconv.Itoa(embedding.TruncatePromptTokens),
		strconv.FormatBool(embedding.SupportsDimensionOverride),
		strconv.FormatBool(params.SupportsVision),
	}, "\x1f")
}

func normalizeModelKeyPart(value string) string {
	return strings.ToLower(strings.TrimSpace(value))
}

func normalizeModelURLKeyPart(value string) string {
	return strings.TrimRight(normalizeModelKeyPart(value), "/")
}
