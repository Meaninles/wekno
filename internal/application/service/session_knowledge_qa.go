...
func (s *sessionService) KnowledgeQAByEvent(ctx context.Context,
	...
	for _, eventType := range eventList {
		...
		if err == chatpipeline.ErrSearchNothing {
			if hasPromptOnlyContext(chatManage) {
				continueWithoutRetrieval(ctx, chatManage, eventType, stageDuration)
				continue
			}
...
func hasPromptOnlyContext(chatManage *types.ChatManage) bool {
	if chatManage == nil {
		return false
	}
	if strings.TrimSpace(chatManage.QuotedContext) != "" {
		return true
	}
	if strings.TrimSpace(chatManage.ImageDescription) != "" && !chatManage.ChatModelSupportsVision {
		return true
	}
	return len(chatManage.Attachments) > 0
}

func continueWithoutRetrieval(ctx context.Context, chatManage *types.ChatManage, eventType types.EventType, duration time.Duration) {
	chatManage.Intent = types.IntentChitchat
	chatManage.SearchResult = nil
	chatManage.RerankResult = nil
	chatManage.MergeResult = nil
	common.PipelineWarn(ctx, "Pipeline", "stage_continue_without_retrieval", map[string]interface{}{
		"event":       string(eventType),
		"duration_ms": duration.Milliseconds(),
		"reason":      "search_nothing_with_prompt_context",
	})
}
