...
func (s *Service) StartArtifactHousekeeping() {
	...
	go func() {
		...
		interval := time.Duration(envInt("AGENT_RUNTIME_ARTIFACT_HOUSEKEEPING_SECONDS", 60)) * time.Second
...
func (s *Service) cleanupStaleArtifactUploads(ctx context.Context) error {
	cutoff := time.Now().UTC().Add(-time.Duration(
		envInt("AGENT_RUNTIME_ARTIFACT_STALE_UPLOAD_MINUTES", 60),
	) * time.Minute)
	var rows []Artifact
...
func (s *Service) deleteArtifactPhysical(ctx context.Context, filePath string) error {
	...
	if s.artifactStore == nil || !s.artifactStore.Owns(filePath) {
		return errors.New("artifact is outside the configured object storage namespace")
	}
	err := s.artifactStore.Delete(ctx, filePath)
	if err != nil && !os.IsNotExist(err) && !artifactNotFound(err) {
		return err
	}
	return nil
}
