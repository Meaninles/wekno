...
func artifactUploadMaxBytes() int64 {
	mb := envInt("AGENT_RUNTIME_ARTIFACT_UPLOAD_MAX_MB", 128)
...
func (s *Service) stageArtifact(
	...
	if run.TenantID != meta.TenantID || run.UserID != meta.UserID || run.SessionID != meta.SessionID || run.MessageID != meta.AssistantMessageID {
		return nil, fmt.Errorf("artifact run identity mismatch")
	}
	...
	if result.RowsAffected != 1 {
		...
		// left behind.
		if err := s.artifactStore.Delete(ctx, row.FilePath); err != nil && !artifactNotFound(err) {
...
func sidecarArtifactFromRow(row *Artifact) *SidecarArtifact {
	...
	return &SidecarArtifact{
		...
		ArtifactID:  row.ID,
		DownloadURL: "/api/v1/custom/agent-runtime/artifacts/" + row.ID + "/download",
