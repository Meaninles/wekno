...
func (s *Service) callTool(ctx context.Context, req ToolCallRequest) (*ToolCallResponse, error) {
	...
	err = s.db.WithContext(persistCtx).Transaction(func(tx *gorm.DB) error {
		...
		if len(current.References) > 0 {
			response.CitationOutputContract = sourcerefs.TerminalCitationInstruction()
		}
...
func (s *Service) prefetch(ctx context.Context, id string, epoch int64) (*ToolCallResponse, error) {
	row, err := s.ownedRun(ctx, id, epoch)
	if err != nil {
		return nil, err
	}
	args, err := json.Marshal(map[string]any{"queries": []string{payloadQuery(row)}})
	if err != nil {
		return nil, err
	}
