...
func (s *Service) callTool(ctx context.Context, req ToolCallRequest) (*ToolCallResponse, error) {
	...
	} else if req.ToolName == ToolInspectInputImage {
		result, callErr = s.inspectInputImage(toolCtx, row, requestPayload, arguments, config)
	} else {
