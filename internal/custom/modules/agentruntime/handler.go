...
func validInternalAPIKey(c *gin.Context) bool {
	expected := strings.TrimSpace(os.Getenv("AGENT_RUNTIME_API_KEY"))
	if expected == "" {
		logger.Errorf(c.Request.Context(), "AGENT_RUNTIME_API_KEY is not configured; refusing general-agent internal callback")
		return false
	}
