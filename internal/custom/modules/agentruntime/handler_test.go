...
func TestCallToolRequiresInternalAPIKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			router := gin.New()
			router.POST("/api/v1/custom/agent-runtime/internal/tools/call", NewHandler(nil).CallTool)
			req := httptest.NewRequest(http.MethodPost, "/api/v1/custom/agent-runtime/internal/tools/call", strings.NewReader(`{}`))
...
func TestCheckpointControlRequiresInternalKey(t *testing.T) {
	t.Setenv("AGENT_RUNTIME_API_KEY", "test-internal-key")
	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.POST("/runs/:operation", NewHandler(nil).RunControl)
	for _, auth := range []string{"", "Bearer wrong", "Bearer test-internal-key"} {
		req := httptest.NewRequest(http.MethodPost, "/runs/checkpoint", strings.NewReader(`invalid-json`))
		req.Header.Set("Content-Type", "application/json")
