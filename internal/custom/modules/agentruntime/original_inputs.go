...
func (s *Service) originalInputFileSpecs(ctx context.Context, req *types.QARequest, runID string) ([]OriginalInputFileSpec, error) {
	...
	for _, item := range req.OriginalInputFiles {
		...
		if err != nil {
			return nil, fmt.Errorf("invalid original input file %s: %w", item.FileName, err)
		}
		out = append(out, spec)
	}
...
func (s *Service) selectedKnowledgeOriginalInputSpec(
	...
	if knowledge == nil || strings.TrimSpace(knowledge.FilePath) == "" {
		return OriginalInputFileSpec{}, fmt.Errorf("选中的知识库条目不是可传给 Agent Harness 的原始文件：%s", knowledgeID)
	}
...
func originalInputMaxBytes() int64 {
	mb := envInt("AGENT_RUNTIME_ORIGINAL_INPUT_MAX_MB", int(defaultOriginalInputMaxBytes/1024/1024))
...
// Agent Harness original-input path prefix.
func originalInputStorageProvider() string {
	provider := strings.ToLower(strings.TrimSpace(os.Getenv("AGENT_RUNTIME_ORIGINAL_INPUT_STORAGE_PROVIDER")))
	if provider == "" {
		provider = strings.ToLower(strings.TrimSpace(os.Getenv("AGENT_RUNTIME_ORIGINAL_INPUT_PROVIDER")))
	}
	if provider != "" {
		return provider
	}
...
func newOriginalInputFileService(provider string) (interfaces.FileService, error) {
	...
	case "minio":
		endpoint := strings.TrimSpace(os.Getenv("AGENT_RUNTIME_ORIGINAL_INPUT_MINIO_ENDPOINT"))
		if endpoint == "" {
			endpoint = strings.TrimSpace(os.Getenv("MINIO_ENDPOINT"))
		}
		accessKey := strings.TrimSpace(os.Getenv("AGENT_RUNTIME_ORIGINAL_INPUT_MINIO_ACCESS_KEY_ID"))
		if accessKey == "" {
			accessKey = strings.TrimSpace(os.Getenv("MINIO_ACCESS_KEY_ID"))
		}
		secretKey := strings.TrimSpace(os.Getenv("AGENT_RUNTIME_ORIGINAL_INPUT_MINIO_SECRET_ACCESS_KEY"))
		if secretKey == "" {
			secretKey = strings.TrimSpace(os.Getenv("MINIO_SECRET_ACCESS_KEY"))
		}
		bucket := strings.TrimSpace(os.Getenv("AGENT_RUNTIME_ORIGINAL_INPUT_BUCKET"))
		if bucket == "" {
	...
	case "obs":
		endpoint := strings.TrimSpace(os.Getenv("AGENT_RUNTIME_ORIGINAL_INPUT_OBS_ENDPOINT"))
		if endpoint == "" {
			endpoint = strings.TrimSpace(os.Getenv("OBS_ENDPOINT"))
		}
		region := strings.TrimSpace(os.Getenv("AGENT_RUNTIME_ORIGINAL_INPUT_OBS_REGION"))
		if region == "" {
		...
		accessKey := strings.TrimSpace(os.Getenv("AGENT_RUNTIME_ORIGINAL_INPUT_OBS_ACCESS_KEY"))
		if accessKey == "" {
			accessKey = strings.TrimSpace(os.Getenv("OBS_ACCESS_KEY"))
		}
		secretKey := strings.TrimSpace(os.Getenv("AGENT_RUNTIME_ORIGINAL_INPUT_OBS_SECRET_KEY"))
		if secretKey == "" {
...
func originalInputPathPrefix() string {
	return strings.TrimSpace(os.Getenv("AGENT_RUNTIME_ORIGINAL_INPUT_PATH_PREFIX"))
}
