...
const (
	generationMarker          = "[WEKNORA_DIALOGUE_CONTINUITY_V9]"
	rewriteMarker             = "[WEKNORA_DIALOGUE_INTENT_V9]"
	turnSemanticMarker        = "[WEKNORA_CURRENT_TURN_SEMANTICS_V9]"
...
- Preserve the source's unresolved label exactly. Unknown/not supplied and pending/awaiting are different states: never substitute one for the other, and never rewrite either as "none", "not applicable", "ready", "not-started", or "completed". Pending may appear only when the user or trustworthy evidence explicitly chose a pending value.
...
- Resolve explicit updates chronologically: newer values may retire conflicting older values. Keep active facts, retired facts, unknown facts, explicitly pending facts, source attribution, requested output scope, and action boundaries distinct.
...
- Preserve unresolved-state polarity: paraphrases may be normalized only within the same class. Unknown/not supplied must not become pending/awaiting, and an explicitly pending value must not become not-started or incomplete. Only an explicit source update changes classes.
...
- Keep unresolved labels in their source class. Unknown/not supplied and pending/awaiting are not interchangeable; pending may be output only when the exact source says pending. Only an explicit user update changes the class.
...
- Keep active facts, retired facts, unknown facts, explicitly pending facts, source attribution, output scope, and action boundaries semantically distinct when the user requests them; do not force state sections onto unrelated knowledge questions.
