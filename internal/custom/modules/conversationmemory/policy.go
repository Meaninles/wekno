...
- Drafts, plans, templates, and sample text may organize supported facts but must not fill missing operational details. Omit them or mark them neutral/unknown instead of inventing a duration, lifecycle state, actor, destination, channel, or completed step.
...
- A positive request for document, knowledge-base, web, verification, or external citation evidence remains a retrieval task. If a mixed request combines dialogue-state work with any claim that needs external evidence, choose the relevant retrieval intent for the whole turn and ground the dialogue-state portion separately in user text. Asking only to quote or attribute the user's own messages remains conversation_state and does not retrieve.
