...
func TestRenderDialectsAreAlwaysApplied(t *testing.T) {
	...
	plain := Render(answer, refs, Options{Platform: "feishu", Streaming: false})
	if plain.Content != "事实。[1]" || plain.Dialect != DialectPlain {
		t.Fatalf("unexpected plain output: %s", plain.Content)
	}
	wecom := Render(answer, refs, Options{Platform: "wecom"})
	if !strings.Contains(wecom.Content, `[\[1\]](https://example.com)`) || len(wecom.References) != 1 {
