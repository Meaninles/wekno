...
const (
	...
	OperationStateCleanup   = "cleanup_pending"
	OperationStateRollback  = "rollback_pending"
...
type Operation struct {
	IdempotencyKey   string            `json:"-" gorm:"type:varchar(64);default:null;uniqueIndex"`
	InputMetadata    types.JSON        `json:"-" gorm:"type:json"`
	InputTagIDs      types.StringArray `json:"-" gorm:"type:json"`
	LeaseOwner       string            `json:"-" gorm:"type:varchar(36)"`
...
// OperationInput is a short-lived durable spool. It is committed with the
// accepted operation, and removed once native ingestion owns the source.
// Keeping it separate prevents status/list/audit reads from loading file bytes.
type OperationInput struct {
	OperationID string `gorm:"type:varchar(36);primaryKey"`
	Data        []byte `json:"-"`
}

func (OperationInput) TableName() string { return "custom_kbmanager_inputs" }
