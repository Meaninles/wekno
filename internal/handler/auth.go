...
func runCustomProvisioner(ctx context.Context, user *types.User) {
	if customProvisionerHook == nil || user == nil {
		return
	}
	if err := customProvisionerHook(ctx, user); err != nil {
		logger.Warnf(ctx, "[custom] failed to provision user %s: %v", user.Username, err)
	}
}
...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	logger.Infof(ctx, "User registered successfully: %s", secutils.SanitizeForLog(user.Username))
...
func (h *AuthHandler) Login(c *gin.Context) {
	...
	runCustomProvisioner(ctx, response.User)
...
func (h *AuthHandler) GetCurrentUser(c *gin.Context) {
	...
	if activeTenantID > 0 {
		...
		if err != nil {
			logger.Warnf(ctx, "Failed to get tenant info for user %s, tenant ID %d: %v", user.Username, activeTenantID, err)
...
func (h *AuthHandler) UpdateMyPreferences(c *gin.Context) {
	...
	if err != nil {
		logger.Errorf(ctx, "Failed to update preferences for user %s: %v", user.Username, err)
...
func (h *AuthHandler) ChangePassword(c *gin.Context) {
	...
	logger.Infof(ctx, "Password changed successfully for user: %s", user.Username)
	c.JSON(http.StatusOK, gin.H{
		"success": true,
...
func (h *AuthHandler) AutoSetup(c *gin.Context) {
	...
	user, _ := h.userService.GetUserByUsername(ctx, defaultUsername)
	if user == nil {
...
func (h *AuthHandler) ValidateToken(c *gin.Context) {
	...
	logger.Infof(ctx, "Token validated successfully for user: %s", user.Username)
	c.JSON(http.StatusOK, gin.H{
		"success": true,
