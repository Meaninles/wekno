...
// handleAgentProgress handles generic user-visible agent progress events.
func (h *AgentStreamHandler) handleAgentProgress(ctx context.Context, evt event.Event) error {
	data, ok := evt.Data.(event.AgentProgressData)
	if !ok {
		return nil
	}
	...
	if toolCallID == "" {
		toolCallID = fmt.Sprintf("agent-progress-%d", time.Now().UnixNano())
	}
	...
	if err := h.streamManager.AppendEvent(h.ctx, h.sessionID, h.assistantMessageID, interfaces.StreamEvent{
		ID:        evt.ID,
		Type:      types.ResponseTypeAgentProgress,
		Content:   content,
		Done:      done,
		Timestamp: time.Now(),
		Data:      metadata,
...
func (h *AgentStreamHandler) handleToolCall(ctx context.Context, evt event.Event) error {
	...
	if !data.PreserveAnswer {
		// Any answer text streamed before a real tool call was a non-terminal
		// round's preamble, not the final answer. Post-answer attachment events
		// opt out via PreserveAnswer so generated file cards do not erase the
	...
	metadata := map[string]interface{}{
		...
		"preserve_answer": data.PreserveAnswer,
	}
...
func (h *AgentStreamHandler) handleComplete(ctx context.Context, evt event.Event) error {
	...
	if h.finalAnswer == "" && data.FinalAnswer != "" && !h.hasStreamedAnswerEventLocked(ctx) {
...
func (h *AgentStreamHandler) hasStreamedAnswerEventLocked(ctx context.Context) bool {
	...
	events, _, err := h.streamManager.GetEvents(ctx, h.sessionID, h.assistantMessageID, 0)
	if err != nil {
		logger.GetLogger(h.ctx).Warnf("failed to inspect streamed answer events before fallback: %v", err)
		return false
	}
	for _, evt := range events {
		if evt.Type == types.ResponseTypeAnswer && evt.Content != "" {
			return true
		}
	}
	return false
