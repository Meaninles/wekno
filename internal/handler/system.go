...
//
// Either user_id (UUID) or username must be supplied. When both are present
// user_id wins because explicit IDs are unambiguous.
//
...
// @Description  Identify the user by username or user_id (UUID, for API clients).
// @Tags         System Admin
// @Accept       json
// @Produce      json
...
func (h *SystemHandler) PromoteUserToSystemAdmin(c *gin.Context) {
	...
	if userID == "" && username == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Either user_id or username is required"})
		return
	}

	// Resolve user. user_id takes priority when both are sent.
	var (
		user *types.User
		err  error
	)
	...
	default:
		user, err = h.userSvc.GetUserByUsername(ctx, username)
	}
	if err != nil {
		logger.Errorf(ctx, "Error fetching user (id=%q username=%q): %v", userID, username, err)
