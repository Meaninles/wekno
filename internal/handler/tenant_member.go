...
// Username is the user-facing member identifier; the invitee must already
// have an account.
...
// ListMembers godoc
// @Summary      列出租户成员
// @Description  分页返回当前租户内 active 成员（含每位成员的角色、用户名、头像）；支持 q 按用户名筛选
// @Tags         租户成员
// @Produce      json
// @Param        id         path   string  true   "租户 ID"
// @Param        q          query  string  false  "按用户名模糊筛选"
...
func (h *TenantMemberHandler) ListMembers(c *gin.Context) {
	...
	// best-effort — a transient batch error degrades to "no username
	// on this page" rather than dropping rows, so dangling
...
// @Description
//
//	Owner 通过 username 直接把用户作为 active 成员添加进当前租户。
//
	...
	if err != nil {
		...
		if errors.Is(err, apprepo.ErrUserNotFound) {
			c.Error(apperrors.NewNotFoundError(
				"user with this username is not registered; ask them to sign up first"))
			return
		}
		logger.Errorf(ctx, "GetUserByUsername failed: username=%s err=%v",
			secutils.SanitizeForLog(username), err)
