...
// handler reaches: GetUserByUsername (AddMember translation) and
...
func TestTenantMember_ListMembers_HappyPath(t *testing.T) {
	...
	us := &stubMemberUserService{
		getByID: func(_ context.Context, id string) (*types.User, error) {
			return &types.User{ID: id, Username: id}, nil
		},
	}
	...
	// Hydration must have populated username so the UI can render members.
	if resp.Data.Members[0].Username == "" {
		t.Fatalf("expected hydrated username, got empty")
	}
}
...
func TestTenantMember_AddMember_HappyPath(t *testing.T) {
	...
	us := &stubMemberUserService{
		getByUsername: func(_ context.Context, username string) (*types.User, error) {
			return &types.User{ID: "u-bob", Username: username}, nil
		},
	}
	h := newTestMemberHandler(ms, us)

	body := map[string]any{"username": "bob", "role": "contributor"}
...
func TestTenantMember_AddMember_UnknownUsernameReturns404(t *testing.T) {
	...
	us := &stubMemberUserService{
		getByUsername: func(_ context.Context, _ string) (*types.User, error) {
			return nil, apprepo.ErrUserNotFound
		},
	}
	h := newTestMemberHandler(&stubMemberService{}, us)

	body := map[string]any{"username": "ghost", "role": "viewer"}
	w := doJSON(t, memberTestRouter(h), http.MethodPost, "/tenants/1/members", body, "u-owner")
	if w.Code != http.StatusNotFound {
		t.Fatalf("unknown username must surface as 404, got %d body=%s", w.Code, w.Body.String())
	}
}
...
func TestTenantMember_AddMember_DuplicateMaps409(t *testing.T) {
	...
	us := &stubMemberUserService{
		getByUsername: func(_ context.Context, _ string) (*types.User, error) {
			return &types.User{ID: "u-bob", Username: "bob"}, nil
		},
	}
	h := newTestMemberHandler(ms, us)
...
func TestTenantMember_AddMember_InvalidRoleRejectedUpfront(t *testing.T) {
	...
	body := map[string]any{"username": "bob", "role": "wizard"}
	w := doJSON(t, memberTestRouter(h), http.MethodPost, "/tenants/1/members", body, "u-owner")
	if w.Code != http.StatusBadRequest {
...
func TestTenantMember_ListMembers_UsesBatchedUserLookup(t *testing.T) {
	...
	us := &stubMemberUserService{
		getByIDs: func(_ context.Context, ids []string) (map[string]*types.User, error) {
			...
			for _, id := range ids {
				out[id] = &types.User{ID: id, Username: id}
			}
			return out, nil
		},
		getByID: func(_ context.Context, _ string) (*types.User, error) {
