...
func (b *SimpleFormatReader) Read(_ context.Context, req *types.ReadRequest) (*types.ReadResult, error) {
	...
	case ft == "md" || ft == "markdown":
		return &types.ReadResult{MarkdownContent: textencoding.DecodeText(req.FileContent)}, nil
	case ft == "txt" || ft == "text":
		return &types.ReadResult{MarkdownContent: textencoding.DecodeText(req.FileContent)}, nil
	case ft == "csv":
	...
	case ft == "json":
		md, err := jsonToMarkdown(textencoding.DecodeTextBytes(req.FileContent))
		if err != nil {
...
func csvToMarkdown(data []byte) (string, error) {
	reader := csv.NewReader(strings.NewReader(textencoding.DecodeText(data)))
