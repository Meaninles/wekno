...
const langfuseRerankMetricDocs = 8
...
func (l *langfuseReranker) Rerank(ctx context.Context, query string, documents []string) ([]RankResult, error) {
	...
	genCtx, gen := mgr.StartGeneration(ctx, langfuse.GenerationOptions{
		...
		Input: map[string]interface{}{
			"query_runes":      len([]rune(query)),
			"query_sha256":     textDigest(query),
			"document_count":   len(documents),
			"document_metrics": documentMetrics(documents, langfuseRerankMetricDocs),
		},
...
func documentMetrics(docs []string, n int) []map[string]interface{} {
	if len(docs) < n {
		n = len(docs)
	}
...
func summarizeResults(results []RankResult, documents []string, n int) []map[string]interface{} {
	...
	for i := 0; i < n; i++ {
		...
		if idx >= 0 && idx < len(documents) {
			row["document_length"] = len([]rune(documents[idx]))
			row["document_sha256"] = textDigest(documents[idx])
		}
		out = append(out, row)
	}
	return out
