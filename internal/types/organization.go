...
// are the primary identity; UserID / Username / Avatar describe the
// representative user (informational, may be empty if the rep user was
// soft-deleted). RepresentativeUserID is the same value as UserID, kept
// as an explicit alias so frontends can stop relying on the misleading
// user_id field name.
