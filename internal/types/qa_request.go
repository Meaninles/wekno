...
type QARequest struct {
	...
	SessionUploadKnowledgeIDs []string            // Server-validated current/historical session uploads; never accepted directly from request JSON
	...
	ProfessionalSkillNames    []string            // Per-request professional skill names selected in the input bar
	...
	OriginalInputFiles        []OriginalInputFile // Runtime-only original file descriptors for Agent Harness
}
...
// OriginalInputFile describes a byte-verified original file copy that the Agent Harness must download into its run directory before the SDK starts.
