...
// with the human-facing user fields the management UI needs (username,
// avatar). It is intentionally NOT the GORM model: returning the model
// directly would leak DeletedAt/UpdatedAt and lock the DB schema into the
// public API. Use this for `/tenants/:id/members` only.
