...
// extractTableNamesFromPgQuery extracts real table names from FROM clauses using
// the pg_query parse tree. CTE aliases are query-local names, so they are
// excluded from the returned table list while the tables referenced inside CTE
// bodies are still included.
func extractTableNamesFromPgQuery(selectStmt *pg_query.SelectStmt) []string {
	...
	extractTableNamesFromSelectStmt(selectStmt, nil, &tables, tableMap)

	return tables
}

func extractTableNamesFromSelectStmt(selectStmt *pg_query.SelectStmt, inheritedCTEs map[string]bool, tables *[]string, tableMap map[string]bool) {
	if selectStmt == nil {
		return
	}

	ctes := make(map[string]bool, len(inheritedCTEs))
	for name := range inheritedCTEs {
		ctes[name] = true
	}
	if selectStmt.WithClause != nil {
		for _, cteNode := range selectStmt.WithClause.Ctes {
			cte := cteNode.GetCommonTableExpr()
			if cte != nil && cte.Ctename != "" {
	...
	for _, fromItem := range selectStmt.FromClause {
		extractTableNamesFromNode(fromItem, ctes, tables, tableMap)
	}
}

func extractTableNamesFromQueryNode(node *pg_query.Node, ctes map[string]bool, tables *[]string, tableMap map[string]bool) {
	if node == nil {
		return
	}
	if selectStmt := node.GetSelectStmt(); selectStmt != nil {
		extractTableNamesFromSelectStmt(selectStmt, ctes, tables, tableMap)
	}
}

func addExtractedTableName(tableName string, ctes map[string]bool, tables *[]string, tableMap map[string]bool) {
	tableName = strings.TrimSpace(tableName)
	if tableName == "" {
		return
	}
	if ctes[strings.ToLower(tableName)] {
		return
	}
...
func extractTableNamesFromNode(node *pg_query.Node, ctes map[string]bool, tables *[]string, tableMap map[string]bool) {
	if node == nil {
		return
	}
	...
	if rangeVar := node.GetRangeVar(); rangeVar != nil {
		addExtractedTableName(rangeVar.Relname, ctes, tables, tableMap)
		return
	}

	// Handle JoinExpr (JOIN)
	if joinExpr := node.GetJoinExpr(); joinExpr != nil {
		extractTableNamesFromNode(joinExpr.Larg, ctes, tables, tableMap)
		extractTableNamesFromNode(joinExpr.Rarg, ctes, tables, tableMap)
		return
	}

	// Handle RangeSubselect (subquery in FROM)
	if rangeSubselect := node.GetRangeSubselect(); rangeSubselect != nil {
		extractTableNamesFromQueryNode(rangeSubselect.Subquery, ctes, tables, tableMap)
		return
	}
}
