-- Custom migration: general-agent artifacts
-- Runtime development environments also execute the same idempotent DDL from
-- internal/custom/modules/generalagent/migration.go so custom modules stay
-- bootstrappable even though the global migrator only scans migrations/versioned.
...
-- General-agent artifacts no longer expose or enforce file-count / extension
-- allowlist settings. Strip stale keys so the persisted config matches the UI.
